        ...
        >
          ...
          <Section icon={Briefcase} title="Employment">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                {(() => {
                  const isHod = form.extraRoles.includes('hod');
                  const needsDept = isHod && !form.departmentId;
                  return (
                    <>
                      <label className={`${lbl} ${needsDept ? 'text-amber-700' : ''}`}>
                        Department{isHod ? ' *' : ''}
                      </label>
                      <select
                        value={form.departmentId}
                        onChange={e => set('departmentId', e.target.value)}
                        className={`${fCls} ${needsDept ? 'border-amber-400 ring-1 ring-amber-300 focus:ring-amber-400/40' : ''}`}
                      >
                      ...
                      {needsDept && (
                        <p className="text-[11px] text-amber-600 mt-1 font-medium">
                          HOD must be linked to a department
                        </p>
                      )}
                    </>
                  );
