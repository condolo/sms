...
import {
  ...
  Zap, Printer, ChevronRight, History, Download,
} from 'lucide-react';
...
export default function CoverTab({ teachers }) {
  ...
  async function handleDownloadPdf() {
    setDownloading(true);
    try {
      await ttApi.substitutions.coverPdf({ date });
    } catch (err) {
      showT(err?.message ?? 'Failed to download PDF.', 'error');
    } finally {
          ...
          <div className="px-5 py-2.5 border-b border-slate-100 flex items-center justify-between print:hidden bg-slate-50/60">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-50 transition"
              >
