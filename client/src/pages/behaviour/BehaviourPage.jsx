/* ============================================================
   ...
   • Houses: settings-based configuration + house leaderboard
   ============================================================ */
...
import {
  ...
  BarChart3, Flag, Award, Home, Palette, Trophy,
} from 'lucide-react';
import {
...
function HousesTab() {
  ...
  /* ── Load school settings (houses stored here) ──────────── */
  ...
  /* ── Load students for house assignment counts ──────────── */
  const { data: stuData } = useQuery({
  ...
  /* ── Load incidents for house leaderboard ───────────────── */
  ...
  function addHouse() {
    ...
    setNewName(''); setNewColor(DEFAULT_HOUSE_COLORS[updated.length % DEFAULT_HOUSE_COLORS.length]); setAddErr('');
  }
      ...
      {incLoading || settingsLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-slate-400">
          ...
          <p className="text-xs mt-1 text-center max-w-xs">
            {isAdmin ? 'Add houses below. Students can be assigned to a house in their profile.' : 'Houses will appear here once configured by an admin.'}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <span className={`font-bold text-base ${netPos ? 'text-emerald-600' : 'text-red-600'}`}>{netPos ? '+' : ''}{h.net}</span>
                      </div>
                    </div>
                    {/* Progress bar */}
