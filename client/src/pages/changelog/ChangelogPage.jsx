...
const RELEASES = [
  {
    ...
    changes: [
      { type: 'new', text: 'Dashboard is now fully permission-aware — KPI cards, charts, student lists, and Quick Actions only appear for roles that have access to the relevant module' },
      { type: 'new', text: 'Quick Actions grid replaced static hardcoded list with a permission-filtered list — Finance, Admissions, HR, and other restricted actions are hidden from roles without access' },
      { type: 'new', text: 'Quick Actions expanded: HR & Staff, Behaviour, Lessons, Reports, Messages actions added (each hidden/shown per role permissions)' },
      { type: 'new', text: 'Teacher dashboard consolidated into main AppShell — teachers now land on /dashboard with full sidebar nav instead of a standalone portal page' },
      { type: 'new', text: 'Teacher dashboard view: teal hero checklist, live timetable (NOW/NEXT badges), attendance ring, at-risk students, curriculum coverage, lesson plans, my classes, form class, upcoming exams, departments, HR leave/payroll, events, announcements' },
      { type: 'fix', text: 'Roles without student access (teachers, HR, finance officers) no longer see student KPI cards — they see Events and Attendance cards instead' },
      { type: 'fix', text: 'Roles without student access no longer see the "Recently Enrolled" students list or "Students by Gender" chart' },
      { type: 'fix', text: 'Student stats API calls are now gated — they do not fire at all for roles without student access, eliminating empty-data cards' },
      { type: 'fix', text: 'Global teacher record lookup: all teacher-facing routes now resolve teacher records via email (two-step: users → teachers) instead of userId, fixing "Teacher record not found" errors for accounts where the userId link was not set' },
      { type: 'fix', text: 'Teacher birthday widget: fixed teachers getting empty birthday lists caused by the same broken userId lookup in birthdays.js' },
      { type: 'fix', text: 'Lesson plans: all 5 endpoints (GET, POST, PUT, PATCH deliver, DELETE) now use the email-based lookup — teachers can view, create, and manage lesson plans regardless of whether their userId was set' },
      { type: 'fix', text: 'Broken /grades Quick Action link corrected to /exams (no /grades route exists)' },
      { type: 'improvement', text: 'Teacher userId back-fill: when a teacher is looked up via email, their userId is silently written back to the record so the next lookup uses the fast single-query path' },
    ],
  },
  {
    version: '4.39.0',
