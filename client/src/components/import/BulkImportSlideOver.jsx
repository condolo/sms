                ...
                <div className={`flex items-start gap-2.5 rounded-lg px-4 py-3 text-sm border ${
                  ...
                  <div>
                    ...
                    {result.invoicesCreated > 0 && (
                      <p className="text-xs mt-0.5 opacity-80">{result.invoicesCreated} opening fee invoice{result.invoicesCreated !== 1 ? 's' : ''} created</p>
                    )}
                    {result.paymentsCreated > 0 && (
                      <p className="text-xs mt-0.5 opacity-80">{result.paymentsCreated} opening balance payment{result.paymentsCreated !== 1 ? 's' : ''} recorded</p>
                    )}
                    {result.usersCreated > 0 && (
                      <p className="text-xs mt-0.5 opacity-80">{result.usersCreated} login account{result.usersCreated !== 1 ? 's' : ''} created · welcome emails sent</p>
                    )}
                    {result.usersCreated === 0 && result.userAccountError && (
                      <p className="text-xs mt-0.5 text-amber-200">Login accounts could not be created — use HR › Staff to create them manually.</p>
                    )}
                  </div>
                </div>
