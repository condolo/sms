...
function computeNav(moduleConfig, userRole, userPermissions) {
  ...
  const visible = CONFIGURABLE_MODULES
    ...
    .filter(m => {
      ...
      // Module not in permissions map → hidden by default (fail-closed)
      return false;
    })
  ...
  return [
      ...
      .map(sec => ({ label: sec, items: grouped[sec] })),
    {
      ...
      items: [
        ...
        if (item.to === '/settings') {
          return isAdminLevel || (userPermissions && Array.isArray(userPermissions.settings) && userPermissions.settings.length > 0);
        }
        return true;
      }),
    },
