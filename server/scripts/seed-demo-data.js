...
async function seedDemoData() {
  ...
  try {
    ...
    /* 21. Portal demo accounts for students 2–5 (std_demo_1 is covered by seed-demo.js) */
    // Student logins — username = admission number (lowercased), same as the real portal endpoint
    const DEMO_PORTAL_STUDENTS = [
      { id: 'u_demo_s2', studentId: 'std_demo_2', username: 'adm-2026-00002', name: 'James Mwangi'    },
      { id: 'u_demo_s3', studentId: 'std_demo_3', username: 'adm-2026-00003', name: 'Fatima Al-Hassan' },
      { id: 'u_demo_s4', studentId: 'std_demo_4', username: 'adm-2025-00012', name: 'David Mutai'      },
      { id: 'u_demo_s5', studentId: 'std_demo_5', username: 'adm-2025-00015', name: 'Grace Waweru'     },
    ];
    // Parent logins — email login, guardianOf links child
    const DEMO_PORTAL_PARENTS = [
      { id: 'u_demo_p2', studentId: 'std_demo_2', email: 'john.mwangi@gmail.com',    name: 'John Mwangi'      },
      { id: 'u_demo_p3', studentId: 'std_demo_3', email: 'hassan.omar@gmail.com',    name: 'Hassan Omar'       },
      { id: 'u_demo_p4', studentId: 'std_demo_4', email: 'kipchoge.mutai@yahoo.com', name: 'Kipchoge Mutai'    },
      { id: 'u_demo_p5', studentId: 'std_demo_5', email: 'peter.waweru@gmail.com',   name: 'Peter Waweru'      },
    ];

    await Promise.all(DEMO_PORTAL_STUDENTS.map(u =>
      User.updateOne({ id: u.id }, {
        $setOnInsert: {
          id: u.id, schoolId: SCHOOL_ID, name: u.name,
          username: u.username, email: null,
          password: hash, role: 'student', primaryRole: 'student', roles: ['student'],
          studentId: u.studentId,
          isActive: true, mustChangePassword: false,
          passwordChangedAt: nowISO, createdAt: nowISO, updatedAt: nowISO,
        },
      }, { upsert: true })
    ));

    await Promise.all(DEMO_PORTAL_PARENTS.map(u =>
      User.updateOne({ id: u.id }, {
        $setOnInsert: {
          id: u.id, schoolId: SCHOOL_ID, name: u.name,
          email: u.email,
          password: hash, role: 'parent', primaryRole: 'parent', roles: ['parent'],
          studentIds: [u.studentId], guardianOf: [u.studentId],
          isActive: true, mustChangePassword: false,
          passwordChangedAt: nowISO, createdAt: nowISO, updatedAt: nowISO,
        },
      }, { upsert: true })
