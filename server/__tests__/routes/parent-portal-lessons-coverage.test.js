...
describe("GET /api/parent-portal/dashboard/:childId — Today's Timetable day-name casing (2026-09)", () => {
  ...
  // ever. "Today's Timetable" showed nothing for every parent's child,
  // every day, since this route shipped.
...
describe("GET /api/parent-portal/dashboard/:childId — Today's Timetable is stream-scoped (2026-09)", () => {
  // Confirmed while investigating the day-casing bug above: the query had
  // no streamId filter at all — a parent saw EVERY stream's lessons for
  // their child's class merged into "Today's Timetable", not just a
  // display quirk but real, wrong subject/teacher/room information shown
  // for that specific child.
  const today = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday'][new Date().getDay()];

  test("a sibling stream's lesson is NOT shown for this child", async () => {
    mockTimetableDocs = [
  ...
  test("this child's OWN stream's lesson IS shown", async () => {
    mockTimetableDocs = [
  ...
  test('a legacy whole-class slot (no streamId at all) still shows', async () => {
    mockTimetableDocs = [
