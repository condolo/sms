...
router.patch('/leave/:id/resolve', async (req, res) => {
  try {
    ...
    if (!HR_ROLES.has(role)) return E.forbidden(res, 'Only HR or Admin can resolve leave requests');
...
router.get('/payroll', async (req, res) => {
  try {
    ...
    if (!HR_ROLES.has(role)) return E.forbidden(res, 'HR / Admin access only');
