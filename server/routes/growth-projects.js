...
router.get('/', authMiddleware, PLAN, rbac('growth_profile', 'read'), async (req, res) => {
  try {
    ...
    const [docs, total] = await Promise.all([
      tenantModel('growth_projects', tenantContext(req)).find(filter).sort({ startDate: -1, createdAt: -1 }).skip(skip).limit(limit).select('-__v').lean(),
      tenantModel('growth_projects', tenantContext(req)).countDocuments(filter),
    ]);
...
router.post('/', authMiddleware, PLAN, rbac('growth_profile', 'create'), async (req, res) => {
  try {
    ...
    const doc = await tenantModel('growth_projects', tenantContext(req)).create({
      ...data,
      id:                 uuidv4(),
      schoolId,
