...
async function _importStudents(rows, schoolId, userId) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    validRows.push({ r, row, gender, status, parentEmail, schoolEmail, classId, manualAdmNo });
  }

  // Reserve numbers only for rows that don't supply their own admission number
  ...
  const toInsert = validRows.map(({ r, gender, status, parentEmail, schoolEmail, classId, manualAdmNo }) => ({
    id:              uuidv4(),
    schoolId,
