...
router.post('/users/invite', authMiddleware, rbac('settings', 'create'), async (req, res) => {
  try {
    ...
    const roleCheck = await validateAssignableRole(req, role, tenantModel, tenantContext, _isSuperAdmin);
    if (!roleCheck.ok) {
      return res.status(roleCheck.status).json({ success: false, error: { code: roleCheck.status === 403 ? 'FORBIDDEN' : 'VALIDATION_ERROR', message: roleCheck.message } });
    }
  ...
  try {
    ...
    await Promise.all(staff.map(async s => {
      ...
      try {
        ...
        // Same shared validator every role-assigning route uses — see
        // utils/role-validation.js's doc comment for why this was missing here.
        const roleCheck = await validateAssignableRole(req, role, tenantModel, tenantContext, _isSuperAdmin);
...
router.put('/users/:id', authMiddleware, rbac('settings', 'update'), async (req, res) => {
  try {
    ...
    if (role) {
      // Previously a locally-defined BUILTIN_UPDATE_ROLES list that had
      // silently drifted from POST /users/invite's own list (this one was
      // missing 'principal', invite's was missing 'guardian' — neither had
      // both). Now the same shared validator every role-assigning route
      // uses, closing that drift for good.
      const roleCheck = await validateAssignableRole(req, role, tenantModel, tenantContext, _isSuperAdmin);
      if (!roleCheck.ok) {
        return res.status(roleCheck.status).json({ success: false, error: { code: roleCheck.status === 403 ? 'FORBIDDEN' : 'VALIDATION_ERROR', message: roleCheck.message } });
      }
