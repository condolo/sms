...
### Added — real absentee list with guardian contact (`GET /api/attendance/absentees`)
Distinct from the existing `/school-report` (deliberately aggregate-only — its own Mongo aggregation never selects `studentId` at all). This new route returns actual student identities for a date — name, admission number, class/stream, and the student's resolved guardian contact (reusing `guardian-contact.js`'s `resolvePrimaryContact`, the same primaryContact-derivation birthday emails and the legacy parent-portal path already use) — so whoever's looking at it can act immediately instead of going to look the student up separately. Gated the same restrictive way as `/school-report`: `hasExplicitSubGrant('attendance','absentees','read')`, no coarse-grant fallback, plus the usual Attendance floor roles. New client tab: Attendance → **Absentees**, with click-to-call/email links.

### Added — same-day present/absent conflict detection, flagging, and resolution
`_checkAttendanceConflict()` runs after every attendance write (`POST /` and `POST /bulk`) for whichever student(s) it affected: if that student now has BOTH an `absent` and a `present` record for the same date (any class/period), a new `attendance_conflicts` document is created (or, if one's already open for that student+date, its snapshot is refreshed instead of raising a duplicate). Whoever's configured as the **Attendance Conflict Resolver** — a school-assignable role or specific person, reusing `workflow-config.js`'s `{assigneeType,assigneeValue}` primitive that Behaviour Officer already uses, not hardcoded to "admissions_officer" — is notified in-app (new `attendance_conflict` notification event) and can resolve the case by entering a reason, which is stored permanently alongside `resolvedBy`/`resolvedAt` as the durable record the request asked for. New routes: `GET/PUT /api/attendance/conflict-officer-config` (assignment; PUT is admin/superadmin only, mirroring `behaviour.js`'s own officer-config exactly), `GET /api/attendance/conflicts` (open/resolved queue), `PUT /api/attendance/conflicts/:id/resolve`. Access to the queue itself: the configured resolver passes unconditionally, everyone else needs the explicit `attendance__conflicts` grant (Settings → Roles & Permissions → Attendance) — off by default for every role, including admissions_officer, matching this codebase's established practice of shipping a new sensitive capability off and letting a school turn it on explicitly rather than guessing a default. New client tab: Attendance → **Conflicts**, including the resolver-assignment picker (admin/superadmin only).

### Fixed — a real bug that would have hidden this exact conflict from ever being detected
The single-record `POST /api/attendance`'s upsert filter was missing `classId` (present all along in the bulk route's own filter, just below it in the same file) — matching only on `{schoolId, studentId, date}` when `period` was omitted, which is the common case. Marking a student absent for one class, then present for a *different* class the same day, both via this route, silently overwrote the first record instead of creating a second one — erasing the very inconsistency this release's whole conflict-detection feature exists to catch, before it could ever be seen. Found while building that feature, fixed by adding `classId` to the filter, matching the bulk route's own long-standing pattern.

### Fixed — Students status filter completeness (`StudentList.jsx`)
"Suspended" is a real, valid student status with its own badge and chart colour, but had no dropdown option — reachable only via "All students." Added. Separately, the Total/Active stat card and every row of the "By Status" chart are now clickable, jumping straight into that exact status filter — so a gap like "313 total, 311 active" is traceable in one click instead of the admin having to guess which status the other N students are sitting in. No backend change — `GET /students` already supported every status via `?status=`; the client just never surfaced a path to all of them.

### Verified
New `attendance-conflicts.test.js` (26 tests, all DB calls mocked): the classId upsert fix creates two records instead of one overwritten; absent+present same day creates one open conflict with both entries; absent-only or present-only never creates one; different students on the same day aren't conflated; a third conflicting write refreshes the existing open case rather than duplicating it; bulk marking triggers detection identically to single-record marking; notification dispatches to the configured resolver and is skipped (not error) when none is configured; the open/resolved queue and resolve-with-reason flow, including rejecting a resolve with no reason and rejecting a second resolve of an already-resolved case; access control — floor roles, the configured resolver, and an explicit sub-grant all independently pass, everyone else is 403'd; admin-only officer-config write. Full Jest suite: 236 suites, 2383/2383 passing. Client build clean.

Live-verified end-to-end against the real demo-school database: confirmed `/absentees` returns a real student's name, class, and admission number for a date with a genuine absence on file; created two real conflicting attendance records (absent in one class, present in another, same student, same day) via the real API and confirmed a conflict was auto-flagged and appeared correctly in the Conflicts tab with both entries shown; resolved it through the real UI with a reason and confirmed it persisted (`status:'resolved'`, `resolvedBy`, `resolvedAt`, `reason`) and dropped out of the open queue; confirmed the two attendance records were saved as two distinct documents, not one overwriting the other (the classId fix, proven live not just unit-tested); confirmed the Students page's "Inactive" stat row and "Suspended" filter option both work against this school's real data, including clicking straight from "22 active" of "23 total" to the one real inactive student. Test attendance records deleted afterward; the resolved conflict record itself was left in place, since "a durable record" was the explicit point of building it.

## [v5.118.0] — 2026-09-23 — fix(timetable): rooms are now a real FK, not free text — plus a live-caught conflicts-endpoint crash

Raised directly: "when updating the timetable, rooms should be like a dropdown list from the room submodule under timetable... not updated as text, that can make it difficult to know the room conflicts." A dropdown already existed (v4.17) but only as a display convenience — the stored value was still a plain string, matched for double-booking purposes by case-insensitive exact-text equality against the Rooms registry. Two different spellings of the same physical room never registered as a conflict, and renaming a room in the registry silently broke matching for every slot already scheduled before the rename. Asked to close this properly: a real `roomId` FK.

### Added
- `timetable` slots gain an optional `roomId` field (`server/routes/timetable.js`). `_applyRoomLink()` resolves it against the live `rooms` registry at `POST /` and `PUT /:id` time and denormalises `room` (the display name) from it — the two fields can never disagree at the moment they're saved. Mirrors `teaching-assignments.js`'s own established `preferredRoomId`/`_roomQuery` pattern exactly, including the same OID-vs-custom-id defensive `$or`.
- `_checkConflicts`'s room-double-booking check now matches on `roomId` when the incoming slot has one, OR on the existing case-insensitive name match, so a linked slot and a legacy free-text slot that happen to name the same room still correctly collide either way — nothing that used to be caught stops being caught.
- `GET /timetable/conflicts` groups room conflicts by `roomId` when linked (immune to a later rename splitting one real room into two never-colliding groups) and resolves the displayed room name from the CURRENT registry entry, not whatever text was denormalised onto the slot at write time.
- Client (`AddSlotSlideOver.jsx`): the Room `<select>` now stores the room's real id, not its name. A legacy slot with only free-text `room` matching a currently-registered name is silently linked in local form state the moment the registry loads — the next Save carries a real `roomId` without the user touching the field. The free-text fallback path (shown only when the stored text matches no registered room) is unchanged and still available for a genuinely unregistered/ad-hoc space.
- `scripts/backfill-timetable-room-id.js` — one-time backfill linking every existing slot whose free-text `room` is an exact, case-insensitive match for a currently-registered room. Slots matching nothing are left as free text, unchanged.
- Bundled in the same payload-rewrite: `room`/`roomId` are now always sent explicitly by the client (never dropped as `undefined`), fixing a related pre-existing bug where clearing the Room field to blank and saving silently failed to persist the clear (a partial `PUT` treats an absent key as "untouched," not "set to empty").

### Fixed — a real, previously-undetected crash in `GET /timetable/conflicts`, found live while verifying the above
`GET /conflicts`'s class-name enrichment ran `{ _id: { $in: allClassIds } }` unconditionally, but `classId` is always the custom string `id` in practice (e.g. `"cls_demo_f1a"`), never a real Mongo ObjectId — Mongoose's typed `_id` cast throws a `CastError` (500), not a graceful no-match, the moment `$in` is handed even one non-ObjectId string. This has apparently never fired in this demo school's history because no conflict had ever previously made it that far into the response — the badge showing "No conflicts" was, in fact, this endpoint silently failing every time a real conflict existed to enrich. Fixed the same way `_classQuery`/`_roomQuery` already guard this exact case elsewhere in this file: only include the `_id` clause for ids that are actually ObjectId-shaped. Live-verified: before the fix, the endpoint 500'd against the real demo database; after, it correctly returned **159 real, previously-invisible conflicts** (teacher and room double-bookings that had been scheduled all along).

### Verified
New `timetable-room-link.test.js` (12 tests, all DB calls mocked): a valid `roomId` denormalises the live registry name over stale client-sent text; an unknown `roomId` is rejected with 404; no `roomId` still behaves exactly as free text always has; two overlapping slots sharing a `roomId` conflict even when their stored text differs (simulating a rename in between); a registered-room booking still catches a conflict against an old unlinked free-text slot with the same name; two different free-text rooms never conflict; the original free-text exact-match path is unchanged; `PUT` correctly no-ops when `roomId` is omitted, unlinks on an explicit empty string, and re-resolves the name when switched to a different room; `GET /conflicts` groups a shared-`roomId` pair into one conflict and reports the live registry name, and a room renamed between two bookings does not silently split into two never-conflicting groups. Full Jest suite: 235 suites, 2362/2362 passing. Client build clean.

Live-verified end-to-end against the real demo-school database (the classNameMap crash above was found this way, not by inspection): registered a new room ("Lab 2") that 12 existing slots already referenced only as free text; confirmed the slot editor silently auto-linked one of them on open, saved it, and confirmed via direct API read that `roomId` persisted; attempted a new overlapping booking against the same `roomId` and confirmed it was correctly blocked; renamed the room via the real Rooms API and confirmed the exact same overlapping booking was **still** blocked, with the error message now showing the live new name — proving the fix is rename-proof, not just theoretically. Ran the backfill script for real against the demo database afterward (11 of 194 free-text slots matched a registered room and were linked; 183 remain genuine free text, correctly left untouched since no school-registered room matches their spelling).

## [v5.117.5] — 2026-09-23 — feat(lessons): Timetable-aware "lessons not planned this week" reminder

Asked directly, alongside the v5.117.4 fix instruction: "connect this with timetable, once teachers timetable is connected the system is aware how many lessons a week needs to be planned for, and reminds the teacher the lessons not planned for."

### Added
- `GET /api/lessons/plans/week-status?weekStart=` — resolves the caller's real, recurring `timetable` slots (`type: 'lesson'`, `isActive: true`) for a given week, computes each slot's actual calendar date, dedupes by `{classId, subjectId, streamId, date}` so a double period counts as one required lesson, and cross-references against `lesson_plans` for that exact date. Returns `{weekStart, weekEnd, totalRequired, totalPlanned, assignments[], unplanned[]}`. `teaching_assignments` only says who teaches what; the timetable is what actually answers "how many lessons this week" — this endpoint reads the timetable, not the assignments list. Self-service only, matching every other lesson-plan route (a teacher sees their own timetable, not another teacher's).
- Reuses the same `$or: [{teacherId: userId}, {teacherId: teacher.id}]` dual-identity match `teacher-portal.js` already established for the pre-existing (documented, unfixed) teacherId drift between `teaching_assignments`/`timetable` records written under the account `userId` vs. the `teachers` collection's own `id`.
- Client: `LessonsPage.jsx`'s Lesson Plans tab now shows a summary banner ("N of M lessons on your timetable planned this week") sourced from this endpoint, plus a new `LessonPlanCard` metric (kept separate from the existing coverage-tracking `ClassCard` — they show different things: this-week's plan count vs. topics-covered-to-date).
- Quick-plan chips: each unplanned lesson in the banner is directly clickable, opening `LessonPlanSlideOver` pre-filled with that lesson's date via a new `initialDate` prop — bypassing the `myClasses()`-sourced card list entirely. Built because live testing surfaced a concrete case of the teacherId drift above: a real teacher's timetable correctly showed 7 lessons due, but `myClasses()` (an exact single-identity match against `teaching_assignments`) only produced a card for 1 of the 3 real class-subject assignments, because the other 2 were recorded under the other identity. Rather than touching `myClasses()`/`_teacherAssignments` itself (foundational, widely reused, out of scope for this ask), the reminder banner now lets a teacher plan a shown-but-uncarded lesson directly.

### Fixed — real timezone bug in date-only week arithmetic, found while building this
`_weekStartOf(dateStr)` parsed `dateStr + 'T00:00:00'` (local midnight) then called `.toISOString()` (always UTC) — on this server's real timezone, `Africa/Nairobi` (UTC+3), that silently shifted the computed Monday back by one calendar day. Caught by the new test suite itself failing (`_weekStartOf('2026-09-21')` returned `'2026-09-20'`). Fixed by parsing with an explicit `Z` and using `getUTCDay()`/`setUTCDate()` throughout — correct, since these are date-only strings, not real timestamps, and pure UTC arithmetic on them is unambiguous regardless of server timezone. The same fix applied to the new `dateForDay()` helper and to "default to today," which now uses local date-part getters (`getFullYear()`/`getMonth()`/`getDate()`) instead of `.toISOString()`.

**Flagged, not fixed:** `teacher-portal.js` computes `todayISO` the same unsafe way (`new Date().toISOString().slice(0,10)`), affecting "Today's Timetable" / "Today's Work" for roughly a 3-hour window each day in this server's timezone. Same bug class, different file — left untouched since it's outside this feature's scope, but worth a dedicated fix.

### Verified
New `lessons-week-status.test.js` (10 tests, all DB calls mocked): a slot with no matching plan is unplanned on the correct date; a plan on the exact date satisfies it; a plan on a different date in the same week does not; a double period dedupes to one required lesson; a stream-specific slot is tracked separately from a whole-class slot; non-`'lesson'` slot types (break, registration) are excluded; an inactive slot is excluded; a slot recorded under the teachers-collection id is still matched via the dual-identity `$or`; a non-Monday `weekStart` resolves back to that week's real Monday. Full Jest suite: 234 suites, 2350/2350 passing.

Live-verified end-to-end against the real demo-school database via a legitimately self-signed test session (this project's own real `JWT_SECRET` used to sign a token for a real teacher account with no known password — the account's actual credentials were never touched, matching the standing no-real-credentials rule): confirmed the banner correctly read "0 of 7 lessons planned," clicked a quick-plan chip for a lesson absent from that teacher's `myClasses()` cards, confirmed the slide-over opened pre-filled with the right class/subject/date, saved a real plan, watched the banner reactively update to "1 of 7," confirmed the now-planned chip disappeared from the remaining list, then deleted the test record via the real `DELETE /api/lessons/plans/:id` route to leave no residue.
...
### Added
- `GET/PUT /api/lessons/template` — a school's field configuration, stored on the `schools` document (`lessonPlanTemplate`), merged against a fixed `BUILTIN_FIELDS` registry the same "backfill missing keys, never overwrite existing ones" way `SettingsPage.jsx`'s own `_makeDefaultPerms`/`_mergePerms` already do for permissions.
- `PUT` is gated by `hasExplicitSubGrant(req, 'lessons', 'template', 'update')` — a new sub-permission, "Configure Lesson Plan Template," visible in Settings → Roles & Permissions → Lessons — not the plain coarse `lessons:update` grant "Edit Lesson Plan" already carries. `admin`/`superadmin`/`principal`/`deputy_principal` bypass unconditionally, matching every other floor in this file.
- `LessonPlanSchema`/`LessonPlanUpdateSchema` gained `customFields: [{key, label, value}]` — self-contained triples, never looked up against the live template at read time, so a school renaming or deleting a custom field later never rewrites or orphans data already recorded on an existing plan. The same guarantee extends to builtin field labels: every plan now snapshots `fieldLabels` (the effective label for each of the 12 builtin fields) at creation time, so a later relabel never rewrites what an already-authored plan displays as — "all data retrievable for later reference" applies to labels, not just values.
- A new **Template** tab on the Lessons page (visible only with the new permission) to toggle/relabel/require each builtin field and manage custom fields.
- `LessonPlanSlideOver` now renders strictly from the school's live template — disabled fields don't appear, required fields are enforced client-side (and should be considered advisory only; the true validation boundary is still the fields that are actually required server-side... see Known Limitation below), and any custom fields appear as extra inputs. Editing an existing plan preserves any custom-field values whose field was since disabled or removed from the template, rather than silently dropping them on save.
- `lesson-plan-pdf.js` renders every builtin field using the record's own snapshotted `fieldLabels` and appends an "Additional Information" section for any populated `customFields`.

### Known limitation, flagged not fixed
Per-field `required` is currently enforced only client-side (in `LessonPlanSlideOver`) — the server's `LessonPlanSchema` does not yet re-validate a field's `required` flag from the live template at `POST /plans` time. A direct API call (or a stale client) can still submit a plan missing a field the school has marked required. Raised as a follow-up; not blocking, since the fields in question are otherwise fully optional today by original design (the schema itself doesn't distinguish required from optional builtins yet), matching the pre-template behavior exactly.

### Verified
New test `lessons-template.test.js` (8 tests, using the real `rbac.js`/`hasExplicitSubGrant`, same discipline as `students-sensitive-permissions.test.js`): the permission gating (coarse `lessons:update` alone is insufficient; explicit `lessons__template` grant works; both floor roles tested bypass), template merge-with-defaults behavior, and a full save-then-reread round-trip including a custom field. New tests in `lessons-plans.test.js` (+4): `fieldLabels` snapshotting with and without a saved template, `customFields` round-tripping through create, and surviving an unrelated `PUT` untouched. Full Jest suite: 233 suites, 2333/2333 passing.

Live-verified against the real demo-school database: added a custom field ("Cross-curricular links") and relabeled "Lesson / Week Assignment" to "Homework" via the real Template tab UI, confirmed both persisted via a direct re-fetch, then created a real lesson plan referencing the customized template and confirmed via direct inspection that it snapshotted the *live* relabeled "Homework" (not the stale default "Lesson / Week Assignment") and stored the custom field correctly. PDF *generation* was independently verified correct by calling the renderer directly against this real record (confirmed byte-for-byte valid, full content, all sections present) — however, retrieving that same PDF **through the running HTTP server** in this session's sandboxed browser produced a truncated/corrupted file on two attempts. Root-caused as far as practical: MongoDB connection instability (`ECONNRESET`, connection timeouts) was actively occurring in the server logs at the same time, and a raw HTTP client isn't available in this sandbox to fully isolate transport vs. environment. Given the renderer itself is proven correct in isolation, this reads as an artifact of this session's environment rather than the shipped code — but it was not possible to fully rule out a live-server-only issue, so it's flagged here rather than silently assumed fine. Worth a real download-and-open spot-check the next time this is used live.
...
Each fix was traced to the real server-side RBAC gate it should have matched — not just swapped for a same-shaped hardcoded check — and two real, additional gaps were found and fixed along the way.

### Dashboard.jsx — Leadership Analytics panel
`LEADER_ROLES = {superadmin,admin,deputy_principal,section_head}` gated the whole panel, even though `GET /api/analytics/leadership` already enforces a real, live `rbac('analytics','read')` grant (moduleRegistry.js's "Analytics Dashboard" module has existed for a while — the checkbox simply did nothing on the client). Replaced with `can('analytics') || isAdminLevel`, matching the same 4 roles by default with zero client behaviour change for a fresh school, but now genuinely configurable.

**Found while reviewing the default-checkbox-state discipline this session has used for every sensitive permission**: `teacher`'s default-permissions function in SettingsPage.jsx had no explicit rule for `analytics`, so it fell through to a blanket `V` (view) default — even though teacher has no `analytics` grant anywhere server-side. Before this fix that drift was inert (the client never read the checkbox), but wiring Dashboard.jsx to it made it live: the next time a school saved the teacher role via Settings, it would have silently handed every teacher account view access to school-wide fee/attendance/behaviour analytics. Guarded to `N`.

### EventsPage.jsx — Add/Edit/Delete Event
`ADMIN_ROLES = ['superadmin','admin','deputy_principal','timetabler']` gated Add Event, Edit, and Delete behind one flag, even though `events.js` enforces them as 3 separate actions (`create`/`update`/`delete`). Split into `canCreate`/`canEdit`/`canDelete`, each `isAdminLevel || can('events', <action>)` — matches the old default exactly (all 3 already RCUD for those 4 roles) but a role can now be granted, say, Create without Delete.

### HRPage.jsx — the whole "HR & Staff" admin view vs. self-service "My HR"
`HR_ROLES = ['superadmin','admin','hr']` decided which entire page a user saw. Replaced with `isAdminLevel || can('hr','read')`, matching `GET /hr/summary`'s own `rbac('hr','read')` gate. (`isAdminUser` elsewhere in this file, gating "Mark as Paid," was checked and left alone — `hr.js` hardcodes that one deliberately: "who actually releases money is not another configurable chain step," not a Settings-driven gate.)

### MessagesPage.jsx + messages.js — deleting someone else's message
Both the client (`canDelete`) and the server (`DELETE /:id`) independently hardcoded `['superadmin','admin','deputy_principal']` as the only roles allowed to delete a message they didn't send — on top of the ordinary, already-Settings-driven "Delete Own Messages" grant. This was a real server-side gap, not just a client one: a school could check "Delete Own Messages" for any role, but moderating everyone else's messages stayed locked to those 3 roles no matter what. Split into a new sub-permission, `messages__moderate` ("Delete Any Message (Moderation)"), enforced with `hasExplicitSubGrant` (no coarse-grant fallback) behind the same 3-role floor for zero regression. Relabeled the existing `delete` sub to "Delete Own Messages" for clarity, same key, no migration needed.

### CatalogTab.jsx / CurriculumTab.jsx — Subjects & Departments, Curriculum assignment
Both checked `role === 'deputy'` — the legacy alias — never the real `deputy_principal` role key, so a Deputy Principal could never manage subjects, departments, or curriculum assignment from the UI no matter what Settings said. Replaced with `can('subjects', <action>)` calls matching `subjects.js`/`departments.js`/`class-subjects.js`'s real coarse actions (`create`/`update`/`delete`). Also found and fixed, in the same file: `CatalogTab.jsx`'s per-subject Edit/Delete buttons were rendered completely unconditionally (no permission check of any kind), relying entirely on the server's 403 to block the save — now gated like every other action on the page.

### TimetablePage.jsx — found during this pass, not part of the original 6
`ADMIN_ROLES = {'admin','superadmin','deputy','timetabler'}` decided whether a user saw the full whole-school timetable management page or the read-only self-service Portal — again keying off the legacy `'deputy'` alias, never `'deputy_principal'` or `'principal'`. Both roles get full `timetable` RCUD by default (same tier as `timetabler`) yet were being silently routed to the Portal regardless. Replaced with `isAdminLevel || can('timetable','update')`, which correctly separates "manages the whole-school timetable" (timetabler/principal/deputy_principal, update granted) from "views their own schedule" (teacher/section_head, read-only) — the same real distinction the module registry already encodes.

### Verified
Full Jest suite: 230 suites, 2285/2285 passing, including a new `messages-moderate-permission.test.js` (5 tests, using the real `rbac.js`/`hasExplicitSubGrant`) proving the new sub-permission requires its own explicit grant, an explicit grant works, and the 3 floor roles are unaffected. Production client build clean. Live-verified in the browser as Deputy Principal against real demo-school data: the Timetable page now shows the full management UI (Class Grid/Rooms/Cover/Publish) instead of the read-only Portal; Events now shows "Add Event"; Subjects now shows "Add Department"; HR now shows the full "HR & Staff" admin view instead of "My HR" — all previously blocked purely by the `'deputy'`-not-`'deputy_principal'` alias bug, now correctly driven by this school's real, already-granted permissions. Messages' moderation Delete button correctly still shows for Deputy Principal viewing another sender's message (floor bypass), unchanged from before.

Separately noticed, not fixed (raised to the user): this demo school's `role_permissions` data carries years of accumulated drift from incremental module additions — some already-existing sub-permission cells (e.g. `messages__delete`, `events__delete` for Deputy Principal) predate later default-value corrections and were never re-synced, because Settings' "Apply & Save new defaults" banner only backfills genuinely missing keys, by design — it doesn't re-harmonize existing ones. Not a regression from this fix; a pre-existing, separate characteristic of that banner. Also unrelated: `GET /api/timetable/conflicts` and `GET /api/analytics/leadership` 500 in the local dev/demo environment on a Mongoose `CastError` (demo seed IDs like `"cls_demo_f1a"` aren't valid ObjectIds) — pre-existing, reproducible before this change (confirmed via git diff that the affected role was already querying that same endpoint beforehand), unrelated to permissions.
...
### The actual bug
`StudentList.jsx` had one hardcoded flag (`canDelete`, later `canHardDelete` for one action) gating FIVE genuinely different actions — Deactivate, Promote, Duplicate resolution, and both bulk/single Portal Account management — completely bypassing Roles & Permissions. Investigating the server side surfaced something more serious: Deactivate, Promote, and Duplicate-resolution all shared the exact same coarse RBAC action (`students:delete` or `students:update`) as routine, already-granted capabilities — meaning any role holding ordinary "edit a student" or "deactivate a student" rights had *already* satisfied the RBAC check for Purge (permanent, cascading delete) and bulk Promotion too, with only the client's hardcoded button visibility standing between that role and calling those routes directly via the API. Portal-account creation had its own separate, identical hardcoded allowlist server-side.

### Fixed
- **4 new, genuinely independent sub-permissions** under Students: "Promote Students to Next Class," "Manage Student Portal Accounts," "Resolve Duplicate Student Records," "Permanently Delete Students" — each enforced with `hasExplicitSubGrant` (no coarse-grant fallback, the same mechanism already used for report-card publishing, mark-submissions review, and the Attendance School-Wide Report earlier this session), so holding the ordinary coarse action never silently grants these. Admin/superadmin remain an unconditional floor for all four; Principal/Deputy Principal additionally floor for Portal Accounts specifically, matching their pre-existing (unchanged) real access.
- **The existing "Delete Student" sub-permission relabeled "Deactivate Student"** — its real effect (`DELETE /students/:id`) has always been a soft, reversible deactivation, never a permanent delete; the old label read as destructive, which is very plausibly why it went unchecked for roles that needed exactly this and nothing more.
- **`StudentList.jsx`'s hardcoded flags replaced** with real permission checks via the client's existing (previously unused for this) `useAuthStore().can()` helper — each of the 5 buttons now reads the specific permission its matching server route actually enforces.
- **Every system role's default checkbox state reviewed before shipping** (same discipline as every sensitive-permission addition this session) — caught 3 real over-granting risks where a blanket fallback would have pre-checked Promote and/or Portal Accounts for Admissions Officer, Deputy Principal/Principal, and Section Head the next time any of those roles were saved. Fixed with explicit denials; nothing else changed for those roles.
- Two additional, unused-by-the-client routes (`PATCH /:id/deactivate`, `PATCH /:id/reactivate`) had the identical hardcoded allowlist layered on top of their own coarse RBAC check — removed for consistency with the route the UI actually calls.

### Verified
Directly against the real, reported account (Trinitas, Admissions Officer): her existing `students:delete` grant now correctly unblocks Deactivate with zero additional configuration, while all 4 new sensitive sub-permissions correctly remain off until explicitly granted. New test `students-sensitive-permissions.test.js` (15 tests) proves each of the 4 separations holds — coarse grant alone is confirmed insufficient, explicit grant works, floor roles are unaffected — plus confirms Deactivate's simple, unchanged behavior. Full Jest suite: 229 suites, 2280/2280 passing (existing `students-purge-audit.test.js`/`students-parent-account.test.js`, which authenticate as admin, confirmed unaffected). Production client build clean. Live-verified in the browser: the admin floor role's Duplicates/Activate-All-Portals/Promote buttons all render and function exactly as before.

---

## [v5.114.0] — 2026-09-22 — fix(security): Attendance inherited "school-level" scope from a role's OWN module, not its real attendance duty

Direct follow-up, reported live with a screenshot: an Exams Officer who is also a homeroom teacher for one stream saw every class in the school (Demo Class, KG, Year 1–8) in the Attendance class dropdown. Confirmed this was NOT a regression from v5.113.0's Exams/Growth Profile dropdown fix — Attendance's own picker was already correctly using `assignedOnly=true`. The real cause: `exams_officer` (like `admissions_officer`, `finance`, `hr`, `timetabler`, `discipline_committee`) is classified `'school'`-level in `scopeMiddleware.js`'s `ROLE_SCOPE_LEVEL` — correctly unrestricted for THEIR OWN module (an Exams Officer legitimately needs every class for exam administration) — but that same blanket classification was silently inherited by Attendance, which has nothing to do with any of those roles' specialties and everything to do with real teaching/homeroom duty.

### Fixed — a dedicated, narrower floor for Attendance specifically
Only `admin`, `superadmin`, `principal`, `deputy_principal`, `deputy` now stay unrestricted for Attendance. Every other role — including the six that ARE `'school'`-level for their own module — is scoped to their real `teaching_assignments` + homeroom streams for Attendance, computed independently of their broader role classification:

- **New `ScopeEngine.resolveAttendanceScope(req)`** (`server/utils/scopeEngine.js`) — the new floor check, plus real assignment resolution (reusing `scopeMiddleware.js`'s own `_loadAssigned`, now exported, rather than a second copy of the same query) and homeroom-stream folding, all in one function. Deliberately does not use `scopeMiddleware`'s own scope cache (keyed by `userId::schoolId` only, no module dimension) — writing a different value under that same key would have silently corrupted every OTHER module's cached scope for the same user on their very next request.
- **Every scope touchpoint in `attendance.js`** (`GET /`, `GET /summary`, `POST /`, `POST /bulk`, `PUT /:id`, `DELETE /:id`) now sources `req.scope` from `resolveAttendanceScope` instead of the generic scope `scopeMiddleware` populated. Also closed `GET /:id`, which — like the exam-results routes found in v5.113.0 — had no scope check of any kind before this.
- **A new, separate `?attendanceScope=true` query flag** on `classes.js`/`streams.js`'s `GET /`, distinct from the existing `?assignedOnly=true` (left completely unchanged, since Exams/Growth Profile's use of it correctly keeps exams_officer etc. unrestricted for their own domain). `streamsApi`'s equivalent flag, previously also named `assignedOnly` but used exclusively by Attendance, was renamed to `attendanceScope` for consistency — it had exactly one caller, so the rename was unambiguous and safe.
- `GET /school-report`'s own floor-role list (added in v5.112.0) now imports the same `ScopeEngine.ATTENDANCE_FLOOR_ROLES` constant instead of a second, independently-drifting local definition.

### Verified
Directly against real Trinitas production data for the exact reported account (Robert Mukhwana, `exams_officer`, homeroom + subject teacher across 3 real class-stream assignments): `resolveAttendanceScope` now correctly resolves to exactly those 3 classes (Year 4, Year 6, Year 7) — down from all 9 classes in the school — while `resolveAttendanceScope` for a real `admin` account correctly still resolves to `null` (fully unrestricted). New/updated tests: `classes-attendance-scope.test.js` (new, 15 — the exact reported scenario, plus every other affected role and the genuine floor), `attendance-write-scope.test.js` (+8 — the data-layer fix, not just the picker, for `exams_officer` across every write route plus the newly-scoped `GET /:id`), `attendance-tenant-isolation.test.js` and `streams-assigned-only.test.js` (updated mocks/param name to match, no behavior change to what they test). Full Jest suite: 228 suites, 2265/2265 passing — a full regression sweep. Production client build clean.

---

## [v5.113.0] — 2026-09-22 — fix(security): teachers could see, and in two cases fetch data for, classes they don't teach — Growth Profile, Exams/Assessment, and a stale Reports bug

Prompted directly: "some teachers can see attendance of other classes/streams that they don't teach... a teacher should only see their class and streams they have been assigned to in the drop down, strictly anywhere including exams/assessment/growthprofile." Investigated each module named before changing anything. Attendance itself was already correctly scoped (verified, not assumed) — the real gaps were in Growth Profile and the Exams/Assessment class picker, plus one unrelated bug found while checking Growth Profile's own reasoning for being unscoped.

### Confirmed real: Growth Profile had zero teacher-level scope enforcement (most severe)
`GET /growth-profile/:studentId` and its `/academic`/`/behaviour` sub-routes checked only the coarse `growth_profile:read` permission — any teacher holding that grant (the role's own default) could fetch **any student's** Growth Profile by ID, regardless of whether that student is in a class they teach. `scopeEngine.js` already registered `growth_profile` as a classId/stream-aware scoped module; these three routes simply never called it.

This was a *documented*, not accidental, gap — `self-service-scope.js`'s own header comment argued staff access here was "bounded by RBAC, not hard-coded to my own class only... this app's convention elsewhere (attendance, grades)." That claim predates this session's earlier scope work and was no longer true: attendance and grades/assessment/exams have all since been scoped by real teaching assignment. The comment was corrected in place rather than silently overridden, so the history is legible to the next reader.

Fixed by wiring in the scope check `growth_profile` was already registered for: `ScopeEngine.isClassInScope(req, 'growth_profile', student.classId, student.streamId)`, added to all three routes. Deliberately does **not** fold in homeroom/form-teacher streams the way Attendance does — a form teacher's pastoral responsibility for a stream is not academic authority over growth records for a student they don't actually teach a subject to, the same boundary the codebase already draws for grades/assessment. New test `growth-profile-teacher-scope.test.js` (15 tests) exercises the real, unmocked scope-resolution logic, including the tricky stream-only-assignment case.

### Confirmed real: Exams/Assessment's class dropdown showed every class to some teachers
`ExamsPage.jsx` derived its class list from `teaching_assignments`, then fell back to the **entire unfiltered school class list** the moment a teacher had zero recorded assignments — most plausibly a pure homeroom/form teacher with no subject-teaching assignment, or a newly added teacher. Every class-scoped exam and assessment data route was already correctly scoped server-side (confirmed, not assumed), so this didn't leak real exam content through the normal UI — but it directly violated "a teacher should only see their own classes, strictly anywhere," and a teacher acting on a class picked from it hit a dead end. Fixed by replacing the ad-hoc client-side reimplementation with `assignedOnly=true` — the exact same server-side mechanism (`resolveClassPickerScope`) AttendancePage.jsx already uses and this session already proved correct, rather than a second, independently-drifting copy of the same logic. Applied to both the main Exams/Markbook class picker and the separate "Create Exam" slide-over's own class fetch. `GrowthProfileHome.jsx`'s class picker had no scoping attempt at all (worse than Exams' flawed one) — fixed the same way.

### Confirmed real, found while checking the above: two Exams routes had zero server-side scope check
While verifying Exams' data routes were actually safe (not assumed), found `GET /exams/:id/results` and `GET /exams/results/all` — unlike `GET /` and `GET /:id`, which are scoped — had no scope check at all. `/results/all` in particular accepted an arbitrary `?classId=` filter directly, a real IDOR: anyone with plain `exams:read` could pull another class's actual results by classId, independent of what the (already-scoped) exam list ever showed them. Not reachable through the current UI (no client caller uses `/results/all`; `/:id/results`'s only caller only ever receives IDs from the already-scoped list), but a real gap on its own, closed with the same `_examClassScope`/`_examInScope`/`_applySubjectScope` helpers already proven correct for the sibling routes. New tests in `exams-scope.test.js` (+7) pin the exact IDOR this closes.

### Also fixed, found while investigating: Reports & Analytics' Attendance tab has shown "not yet available" for every school, always
Separately reported live via screenshot (Trinitas, superadmin view): Avg Daily Attendance / Days Recorded / Chronic Absent all blank, "Attendance summary not yet available. Mark attendance to see data here." — despite real attendance data existing. Root cause: `ReportsPage.jsx` reads `attSummary.avgRate`/`.daysRecorded`/`.chronicAbsent`/`.byClass`, but `GET /attendance/summary`'s school-wide branch has never computed those fields — only `{total,present,absent,late,authorised,attendanceRate}`. A field-name/shape mismatch, not a data problem, present since the Reports page was built; unrelated to any change this session made until now. Fixed additively — the pre-existing shape (relied on by `Dashboard.jsx`'s own attendance widget) is untouched; the four new fields are computed in the same `$facet` pass over the same already-scoped filter, so this stays one query. Verified against real Trinitas data (449 records, 8 classes, chronic-absence and per-class rates all computed correctly) and live in the browser (demo school: 80% avg, 3 days recorded, 2 chronic absentees, bar chart rendering). New test `attendance-summary-schoolwide.test.js` (6 tests).

### Verified — full regression sweep, no shortcuts given this is a live system
- New tests: `growth-profile-teacher-scope.test.js` (15), `exams-scope.test.js` (+7), `attendance-summary-schoolwide.test.js` (6) — 28 new, all passing.
- Full Jest suite: 227 suites, 2242/2242 passing (zero regressions across the whole codebase, not just the touched files).
- Production client build clean.
- Live-verified in the browser: Reports/Attendance panel renders real numbers; Growth Profile roster and profile navigation work correctly for an unrestricted role; Exams/Markbook class pickers render correctly post-fix.
- The one unexpected 403 hit during live verification (Growth Profile, demo admin) was traced to its root cause before being dismissed, not assumed away: the demo school's `admin` role_permissions document is separately missing the `growth_profile` key entirely (confirmed via direct query) — a pre-existing demo-seed gap, unrelated to this fix. Real production data (Trinitas) has the correct grant and was used to confirm the fix's logic directly.
- Tenancy: every new/changed query already flows through `tenantModel`'s own `_scopedPipeline`/`_scopedFilter` wrapper (confirmed by reading it, not assumed), which independently injects and validates `schoolId` on every `find()`/`aggregate()` call — a second, structural layer of tenant isolation beneath the explicit `schoolId` already in every filter.
...
### The gaps, confirmed before fixing
- `exams.js` never called `scopeMiddleware`/`ScopeEngine` anywhere — `GET /exams` and `GET /exams/:id` were gated only by RBAC (`exams:read`) and `schoolId`. Any teacher could list or open every exam in the school, not just their own class/subject.
- `POST /exams/:id/results`'s only ownership gate was an optional, client-suppliable `exam.ownerId` — the general admin `POST /exams` create path never sets it, so an exam created there had **no ownership check at all**; anyone with `exams:create` could enter its results.
- `assessment.js`'s `GET /marks`, `GET /report`, and `GET /marks/summary` had no scope check either — only `GET /analytics` did. Any `grades:read` holder could list marks, pull a computed report, or see the mark-entry completion grid for any class in the school.
- `report-cards.js`'s `GET /` and `GET /:id` never called `scopeMiddleware`/`ScopeEngine` — `MODULE_SCOPE.report_cards`'s `streamAware: true` entry was configured but never actually wired into a query. A teacher with `grades:read` could list or open any student's full report card (scores, GPA, comments) in the school.
- `GET /report-cards/:id`'s ownership check only covered `'parent'`/`'guardian'` roles — a **`'student'`-role caller had no ownership check at all** and could fetch any other student's report card by id.
- `PUT /report-cards/:id/comments` had no subject-ownership check on `subjectComments`, unlike the `draft-comments` routes covering the same data pre-publish — any `grades:update` holder could overwrite any subject's comment on any published report card.
- `server/utils/subject-scope.js`'s `canWriteSubject`/`unassignedPairs` (the `{classId, subjectId}`-exact check already used by `assessment.js`'s mark-entry writes) only ran `if academic_config.subjectAssignmentEnforced` — an opt-in flag with **no Settings UI anywhere in the app** to turn it on (confirmed by search), so it was permanently off for every real school.

### Fixed
- **Unconditional subject-assignment enforcement** (confirmed explicitly before changing, since it alters live write behavior for every school): `canWriteSubject`/`unassignedPairs` in `server/utils/subject-scope.js` no longer read `subjectAssignmentEnforced` at all — a teacher may now only enter/delete marks, exam results, or report-card subject comments for a `{classId, subjectId[, streamId]}` pair they hold a real `teaching_assignments` record for, at every school, with no opt-out. Management-tier roles remain exempt, unchanged. The `academic_config.subjectAssignmentEnforced` field is kept in the schema for backward compatibility only — nothing honors it as a gate anymore.
- `server/utils/scopeEngine.js` — `MODULE_SCOPE.exams` changed from `{field: 'subjectId', source: 'subjectIds'}` (unused, and the wrong shape — it would have matched a subject across every class in the school) to `{field: 'classId', source: 'classIds'}` (matching grades/assessment/report_cards' own convention; not `streamAware`, since `exam_results` carries no `streamId` at all).
- `server/routes/exams.js` — `GET /` and `GET /:id` now apply the class-level scope plus an exams-local subject-level narrowing (`_applySubjectScope`), since a module can only have one scoped field in `ScopeEngine`. A stream-only-scoped teacher's own class still resolves correctly via a local, homeroom-excluded parent-class fold (`_examClassScope` — deliberately does not reuse `resolveClassPickerScope`, which folds in homeroom/pastoral streams that have no business seeing exam data). `POST /:id/results`'s ownership check now falls back to a real `canWriteSubject` check whenever the caller isn't the recorded `ownerId` and isn't an admin — an exam with no `classId`/`subjectId` at all has nothing to check and is allowed through unchanged.
- `server/routes/assessment.js` — `GET /marks`, `GET /report`, and `GET /marks/summary` now apply the same `ScopeEngine`/`isClassInScope` narrowing `GET /analytics` already used, closing the exact gap `grades.js`'s equivalent routes never had.
- `server/routes/report-cards.js` — `GET /` and `GET /:id` now apply class(+stream) scope for staff callers, and force a `studentId`/`guardianOf` restriction for restricted roles (parent/student/guardian) reaching this admin-facing route directly, rather than trusting `schoolId` alone. Added the missing `'student'`-role ownership check to `GET /:id` (mirroring the existing `parent`/`guardian` check, including the same compliance audit log). `PUT /:id/comments` now runs the same `unassignedPairs` subject check the `draft-comments` routes already had, for `subjectComments` specifically — `classTeacherRemark`/`principalRemark` are unaffected, since those have their own, different authority model.

### Verified
- New test files: `exams-scope.test.js` (14 tests — class/subject/stream-fold narrowing on both GET routes, and every ownership branch on `POST /:id/results`), `assessment-read-scope.test.js` (10 tests), `report-cards-scope.test.js` (14 tests — including the `'student'`-role fix and the `PUT /:id/comments` subject check).
- Existing test files whose fixtures pre-dated unconditional enforcement (`exams-results-grade-scale.test.js`, `exams-mark-conflict.test.js`, `assessment-mark-conflict.test.js`, `assessment-mark-academic-year-adoption.test.js`, plus the two files' own stale "enforcement OFF" test cases) updated to seed a real `teaching_assignments` record, since every write in those files is now correctly gated — none of them were ever testing the subject-scope check itself.
- Full Jest suite: 220 suites, 2183/2183 passing. `verify-rbac-coverage.js` 100% (487/487, no regression). `security-scan.js` clean.
...
### Fixed
- `server/utils/scopeEngine.js` — removed the `streamAware` flag from `classes`'s `MODULE_SCOPE` entry; documented why in a new comment block (a class is the parent of its streams, not a record belonging to one). Added a new shared helper, `resolveClassPickerScope(req)`, that resolves a caller's stream-only assignments to their **parent** classIds (via the `streams` collection) and returns a brand-new scope object — never mutates `req.scope` in place, since `scopeMiddleware.js` caches that exact object per `userId::schoolId` for 5 minutes and every other module's route reads the same cached object on its own next call.
- `server/routes/classes.js`'s `GET /` (`assignedOnly=true` branch — Attendance's class picker) and `server/routes/assessment.js`'s `GET /analytics` (`availableClasses` dropdown) both call this one helper before their own `ScopeEngine.applyToFilter`/`hasNoAssignments` calls, then restore the original `req.scope` immediately after. **The second call site was found during this fix's own verification pass** — a grep for every caller of `applyToFilter(req, 'classes', ...)` turned up `assessment.js` suffering the identical bug for the identical reason, not yet reported but confirmed broken the same way; fixed in the same change rather than left for a second report. Confirmed via the same grep that these are the only two callers anywhere in the codebase, and that `attendance.js`'s/`streams.js`'s own `req.scope.streamIds` usage is correct, unrelated record-level scoping for their own modules — not a third instance of this bug.
- The write routes this picker feeds (`POST /attendance`, `/attendance/bulk`) and the record-level list queries both routes call already narrow to the correct stream on their own and are completely untouched by this change.
...
- 4 new tests in `server/__tests__/routes/classes-assigned-only.test.js` (stream-only teacher sees their stream's parent class; stream-only across two classes sees both; mixing a whole-class grant with a stream-only grant in a different class shows both; a cache-isolation test confirming the merge never writes back into the shared cached scope object) plus 1 new test in `server/__tests__/routes/assessment-analytics.test.js` covering the second call site.
- Full Jest suite: 208 suites, 2059/2059 passing (2054 + 5 new). `verify-rbac-coverage.js` 100% (487/487, no regression — no RBAC/module-gate changes, this is a data-scope-only fix). `security-scan.js` clean.
...
### Root cause
`PUT /api/students/:id` unconditionally deleted `admissionNumber` from every incoming payload, labeled "Immutable server-generated fields" — a blanket rule written for fields that genuinely never change after creation (`schoolId`, `id`, `_v`), but admission number isn't one of those: it can be manually overridden at creation (v5.75.0), so there was never a real reason it couldn't be corrected afterward too. The save request always returned success either way, because nothing about dropping one field made the rest of the update fail — there was simply no error to surface.

### Fixed
- `PUT /api/students/:id` now accepts an admission-number change: unchanged values are a no-op (no duplicate check, nothing rewritten), a blank value is rejected outright (an admission number can't be empty), and a genuine change is checked against every other student in the school first — the exact same collision guard `POST /api/students` already applies — before being saved. A collision returns a clear 409 instead of silently creating the kind of duplicate v5.75.0 was built to prevent.
- No client changes needed — the Student Profile edit form's existing save flow (and its existing success/error toasts) already handle this correctly now that the server stops discarding the field.

### Verified
- 4 new integration tests in `students.test.js`: a genuine change is saved through to the update; resending the same value triggers no duplicate check and writes nothing; a change to a number already used by another student is rejected with 409; blanking it out is rejected with 400. Full server suite 1993/1993. `verify-rbac-coverage.js` 100% (no regression — existing route, no shape change). `security-scan.js` clean. Client production build passes.
...
### Added
- **Categories** (`inventory_categories`) — auto-seeded with ICT/Laboratory/Sports/Furniture/Office/Cleaning/Kitchen on a school's first read, same lazy-seed posture as Behaviour's categories. Fully school-editable; deletion is blocked while items still reference the category.
- **Items** (`inventory_items`) — item code, name, category, quantity, unit, store/location, status. `categoryName` is denormalized and resolved server-side when the caller omits it. `quantity` is stripped from `PUT` — it only ever moves through the transaction ledger below, never a direct field edit, so the historical trail stays trustworthy from day one. Deletion is blocked once an item has stock-transaction history (set its status to `discontinued` instead).
- **Stock Transactions** (`inventory_transactions`, append-only — no PUT, no DELETE) — Receive, Issue, Return, Adjustment. A mistake is corrected with an offsetting adjustment, never by editing history. Adjustment requires both a `direction` (increase/decrease) and a non-empty `reason`. The linked item's `quantity` updates via an atomic `$inc` with a guard condition (`quantity >= -delta`) rather than read-compute-write — this is what actually prevents a lost-update race between two concurrent transactions on the same item, verified directly (an issue for more than current stock is rejected with quantity left untouched; one for exactly the current stock succeeds to zero). If the ledger entry fails to write after the quantity already moved, a compensating `$inc` rolls it back.
- **Requisitions + Procurement** (`inventory_requisitions`) — any user holding the `inventory__requisition` grant can raise one, linked to an existing catalogue item or a brand-new item request by name. Reuses the Workflow Engine exactly like HR's Leave approval: a `currentStepOrder` state machine, `PATCH /:id/advance` (per-instance approver eligibility via `resolveStep()` — not an RBAC gate, matching `hr.js`'s `/leave/:id/advance`), in-app notifications to each step's resolved approvers. Once the chain clears, `POST /:id/fulfill` — gated by the same permission needed to record any stock transaction, deliberately not tied to the approval chain itself — creates the real `receive` transaction, updates the item's quantity, and marks the requisition `fulfilled`.
- New `inventory` RBAC module (`server/config/moduleRegistry.js`) with sub-permissions `view`/`manage`/`transact`/`requisition`/`workflow`. Default grants mirror Medical Centre's posture: admin/principal/deputy_principal get full access; teacher gets only `inventory__requisition` (raise + view own requests, matching the module spec naming Teacher as a requester, not an inventory manager); everyone else — Secretary, Cleaner, Laboratory Technician, per the module's own requester examples — is granted via custom roles as needed, same as Medical Centre's "Nurse" precedent.
- New audit actions: `inventory.item_created`, `inventory.item_deleted`, `inventory.transaction_recorded` (escalated to `warn` severity for adjustments), `inventory.requisition_raised`, `inventory.requisition_step_approved`, `inventory.requisition_step_rejected`, `inventory.requisition_fulfilled`.
- Client: new "Inventory" nav item and page (`client/src/pages/inventory/InventoryPage.jsx` — Items / Stock Transactions / Requisitions / Categories tabs; `components/RequisitionsTab.jsx` includes an approval-chain configuration modal mirroring HR's own, kept as a separate component rather than an extraction since the HR version has leave-specific behavior baked into its markup).

### Verified
- 74 new tests across three files (`inventory-categories-items.test.js`, `inventory-transactions.test.js`, `inventory-requisitions.test.js`) — RBAC on every route, the auto-seed/duplicate-rejection/referential-integrity guards, the insufficient-stock guard at and past its exact boundary, the ledger-write-failure rollback, and — with the real `workflow-config.js` module deliberately left unmocked — the full requisition approval state machine and the step-eligibility-vs-RBAC distinction. Two real bugs were caught by these tests before shipping: a guessed `email.sendGenericNotification` function that doesn't exist anywhere in the codebase (replaced with the proven in-app `_sendSystemMessage` primitive HR's leave chain already uses), and a test mock silently dropping the `$inc` half of the fulfill route's combined `{$inc,$set}` update. Full Jest suite (125 suites / 1175 tests) and a production client build both pass. RBAC coverage scanner: 475/475 (100%).

---

## [v5.32.0] — 2026-08-01 — feat(medical): Medical Centre — Module 1 (Student Medical Profile, Clinic Visits, Alerts, Reports)

A dependency audit preceded implementation, confirming zero duplicate concepts: the medical profile extends the existing Student record in place rather than creating a parallel one, RBAC/audit/notifications/tenant isolation are all the existing platform mechanisms (no new permission system, no new audit log), and Medical Alerts derive from the same Student Medical Profile rather than a separate flagged collection.

### Added
- **Student Medical Profile** — extends `students.medical` (a plain sub-object on the existing Student document, not a new collection): blood group, allergies, chronic conditions, disabilities, vaccinations, general notes, emergency contact, doctor/clinic info, and Parent Medical Consent (`parentConsentGiven`/`parentConsentNotes`, with `recordedAt`/`recordedBy` stamped server-side — never trusted from the client, and only refreshed when the consent value actually changes, so an unrelated edit like updating allergies can't silently overwrite who/when consent was last recorded). The legacy top-level `students.medicalNotes` (still used by the Add Student quick-entry form) now mirrors into `medical.notes` automatically at creation time; `scripts/backfill-student-medical-notes.js` (dry-run capable) covers students created before the change.
- **Clinic Visits** (`server/routes/medical.js`, new `medical_visits` collection) — every visit is its own immutable record (`POST`/`GET /api/medical/visits`; deliberately no `PUT` — "every visit creates a new record, never overwrite" — a data-entry mistake is corrected by soft-deleting the wrong visit and logging a new one). `DELETE` sets `deletedAt`/`deletedBy` and excludes the record from default reads; it is never destroyed. Fields: complaint, observation, action taken, medication given, returned-to-class/sent-home/referred flags, notes, and the recording nurse/user (server-set, not client-supplied). `studentName` is resolved server-side when the caller omits it — proactively avoiding the class of bug fixed in `behaviour_incidents` earlier this session rather than repeating it.
- **Medical Alerts** (`GET /api/medical/alerts`) — condition flags only (severe allergy, asthma, epilepsy, plus a free-text catch-all), never the full profile. The response is hand-built field-by-field from the query result rather than passed through, so a future `.select()` change widening the query can't leak `bloodGroup`/`doctorName`/notes/consent through this route. Scoped by class assignment for teachers via the same `scopeMiddleware`/`ScopeEngine` the main student roster already applies — a teacher sees alerts for their own students, not the whole school.
- **Reports** (`GET /api/medical/reports`) — Visits Today and Visits This Month (real calendar counts, independent of query-string overrides), Common Conditions and Frequent Visitors (aggregated over a configurable window, defaulting to the current month). No advanced medical analytics, per the module spec.
- New `medical` RBAC module (`server/config/moduleRegistry.js`) with sub-permissions `view`/`record`/`delete`/`alerts`/`reports`. Default grants: admin/principal/deputy_principal get full access; teacher gets only the `medical__alerts` sub-grant (condition flags, never full clinic records); everyone else is restricted until a school grants a custom "Nurse" role, matching the module's stated sensitivity ("full medical records remain restricted").
- New audit actions: `student.medical_updated`, `student.medical_consent_recorded` (critical severity, given its legal/compliance weight), `medical.visit_logged`, `medical.visit_deleted`.
- New `medical_visit_logged` parent-facing notification event, reusing the existing guardian-notification dispatcher.
- Client: new "Medical Centre" nav item and page (`client/src/pages/medical/MedicalPage.jsx` — Log Visit / Visits / Alerts / Reports tabs). Student Profile's Medical tab gained Disabilities, Notes, and Parent Medical Consent fields, plus a red critical-alerts banner when any condition flag is set.

### Fixed
- `MedicalInfoSchema` didn't exist on `StudentUpdateSchema` at all before this module's first milestone — the Student Profile's Medical tab had always posted a `medical` payload, but Zod silently stripped the unknown key on every save, so "Edit medical info" was a no-op regardless of what a school entered. Declaring the schema was the fix, not a new field addition.

### Verified
- 34 new tests across four files (`students.test.js` additions, `medical-visits.test.js`, `medical-alerts.test.js`, `medical-reports.test.js`) — RBAC (including the alerts-only teacher case and confirming that grant does NOT also unlock reports), server-side `studentName` resolution, the consent-stamping rules, the no-full-profile-leak response-shape assertion for Alerts, and the class-based scoping for teachers (including a real test-isolation bug found and fixed along the way — `scopeMiddleware`'s 5-minute in-memory scope cache was leaking a stale scope across test cases reusing the same userId). Full Jest suite (122+ suites) and a production client build both pass at each milestone. RBAC coverage scanner: 458/458 (100%) at this module's completion.
