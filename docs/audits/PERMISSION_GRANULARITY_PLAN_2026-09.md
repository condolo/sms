...
| `lock` | 🟢 **REAL — FIXED 2026-09-03, completed 2026-09-05** | **REAL** | Freeze an exam so results can no longer be edited | exams_officer, Principal, Deputy Principal — **not the teacher entering results** | `exams.js` — `POST /:id/lock` AND `PUT /:id { status: 'locked' }`, both now consistent | Split from the old combined "Lock / Unlock Exam" row — see §4a. Admin/superadmin floor preserved; `exams.lock` independently grantable. **2026-09-05:** the dedicated endpoint itself was found still broken for a granted non-floor caller (see §4a's RESOLVED addendum), and the `PUT /:id` alternate path is now permission-consistent with it too — both fixed together. |
| `unlock` | 🟢 **REAL — FIXED 2026-09-03, permission-consistency completed 2026-09-05** | **REAL** | Reverse a lock, with mandatory reason + audit log via the dedicated endpoint | exams_officer, Principal, Deputy Principal | `exams.js:552-566` (dedicated `POST /:id/unlock`, unaffected — was always correct) plus `PUT /:id { status: 'approved' }` from `locked`, now also grant-aware | New row (didn't exist before this fix — the old registry only had one combined `lock` key). Independently grantable from `lock` — proven by test: a role holding only one of the two cannot do the other. **`PUT /:id`'s version of unlock still has no mandatory-reason/audit-log requirement — that request-shape gap remains open, separate from the permission question this row tracks.** |
...
**Implemented (2026-09-03), Option A exactly as proposed above.** Before coding, a mandatory read-only legacy-data check was run (`scripts/audit-exams-lock-legacy-grants.js`) — since the fix changes the MEANING of an already-existing key (`exams__lock` went from representing "Lock/Unlock combined" to "Lock only", with `unlock` becoming a brand-new, separate key), any pre-existing non-empty grant would go from inert to live the moment the code shipped. Real result: **all 3 schools in this environment already had `exams__lock` populated for every role** (an artifact of `_deriveApiPerms` writing a full sub-key snapshot on every Global Role save, not isolated stale data) — specifically, `deputy_principal`, `deputy`, `principal`, and `exams_officer` at every school would gain live "lock exam" authority on deploy (their stored arrays contain `'update'`); `teacher` and `section_head` would not (their arrays are `['read']` only). Assessed as low-risk and arguably a genuine improvement — those are plausible, sensible holders of lock authority, none implausible gain anything, and the operation is reversible and already audited — but recorded here explicitly as a real, previously-undecided access change rather than left silent.

Registry split, exactly as proposed (`server/config/moduleRegistry.js`): `lock` relabeled "Lock Exam", new `unlock` row "Unlock Exam" added. Route changes exactly as proposed, in both `POST /:id/lock` and `POST /:id/unlock` (`server/routes/exams.js`). 12 new tests (`exams-lock-unlock-permission.test.js`), including two independence proofs (a role granted only `lock` cannot `unlock`, and vice versa) and two requirement-preservation proofs (mandatory reason and the `mark_audit_log` write both still fire for an explicitly-granted, non-floor caller). Both checks mutation-tested independently. `TRANSITION_ROLES`/`_checkTransition` and `PUT /:id` were not touched, per Option A.

**Explicitly still open, tracked, not resolved by this fix (2026-09-03):** `PUT /:id { status: 'locked' | 'approved' }` remains a fully working alternate path to the same two transitions, gated only by the unmodified `TRANSITION_ROLES` (admin/superadmin only, regardless of any `exams.lock`/`exams.unlock` grant), and without `/unlock`'s mandatory-reason/audit-log requirements. **The accurate status statement is:** *dedicated Exam Lock/Unlock endpoints are now controlled by Settings; the generic status-transition path is not, and remains a separate, unresolved correctness finding.* Not: "Exam Lock/Unlock permissions are now fully controlled by Settings" — that would overstate what shipped.

**RESOLVED — the permission-consistency part (2026-09-05).** Two things, found together while writing a role-specific user guide for Exams Officer and re-tracing this exact authorization path against real usage rather than trusting the existing test suite's assertions:

1. **The 2026-09-03 fix above was, itself, incomplete in a way this document did not catch.** `POST /:id/lock` computed `hasExplicitSubGrant(req,'exams','lock','update')` correctly, but then called `_checkTransition(exam.status, 'locked', role)` **without** passing that result — so `_checkTransition`'s own `TRANSITION_ROLES.locked = ['admin','superadmin']` immediately re-rejected the very caller the grant check had just approved. A newly-granted, non-floor Exams Officer could therefore NOT actually lock an exam through the dedicated endpoint either — contradicting this document's own "dedicated endpoints are now controlled by Settings" framing at the time it was written. The existing test (`exams-lock-unlock-permission.test.js`'s "THE NEW CAPABILITY" case) only asserted `res.status !== 403`, which the resulting 400 also satisfies, so this passed silently. Fixed by having `/lock` pass its already-computed `hasLockGrant` boolean through to `_checkTransition`, which now honours it for the `locked` target specifically.
2. **The `PUT /:id` alternate path** now also resolves and passes the same `exams.lock`/`exams.unlock` grants into `_checkTransition` when the status change actually targets `locked`, or moves from `locked` to `approved` (unlock) — closing the permission-consistency gap this document flagged as open. `_checkTransition` distinguishes `moderated → approved` (the ordinary Approve action, now open to `exams_officer` via `TRANSITION_ROLES`, see §4b) from `locked → approved` (unlock, still floor-or-grant-gated) by `fromStatus`, so granting ordinary Approve authority can never accidentally also grant unlock authority through the generic path.

**Still explicitly NOT touched, remains open, unrelated to the above:** the reason/audit-log asymmetry — `PUT /:id`'s status-change path still doesn't require a `reason` or write a `mark_audit_log` entry the way the dedicated `/unlock` endpoint does. That is the correctness finding flagged in point 4 above, deliberately out of scope here (a request-shape completeness gap, not a permission-granularity one) — an admin or newly-granted Exams Officer can still unlock via `PUT /:id` with no reason and no audit trail. New tests: `exams-officer-transitions.test.js`, including a strengthened re-test of `POST /:id/lock` that asserts the actual outcome (200 and `exam.status === 'locked'`) instead of only `!== 403`.

### 4b. `exams_officer` excluded from `TRANSITION_ROLES` entirely — found and fixed 2026-09-05

**A distinct finding from §4a**, discovered the same way (writing the Exams Officer role guide and tracing what the role can actually do, rather than what its RBAC grant implies it should be able to do). Separate from the lock/unlock item above, and larger in practical impact:

`TRANSITION_ROLES` (`exams.js:71-80`, before this fix) listed `['teacher','admin','superadmin']` for `in_progress`/`completed` and `['admin','superadmin']` for every other transition (`cancelled`, `moderated`, `approved`, `locked`, `published`, `archived`). **`exams_officer` appeared in none of these lists**, despite holding full `exams:RCUD` in `server/utils/repairPermissions.js`'s `ROLE_DEFAULTS` — the role whose entire stated purpose is administering exams. In practice: an Exams Officer could create an exam and enter marks in the Markbook, but could not move a single exam through `PUT /:id` from `Scheduled` to `In Progress` — not even Start Exam — let alone Moderate, Approve, Publish, or Archive. The client (`ExamsPage.jsx`) compounded this by also hardcoding its button-visibility check to `role === 'admin' || 'superadmin'`, so the gap wasn't even visible as a set of disabled/hidden buttons pointing at a real restriction — it looked like the feature simply wasn't there for that role.

This is architecturally the same class of issue as §4a (a hardcoded, Settings-invisible role list layered underneath the coarse RBAC permission) but affects the whole transition surface rather than just lock/unlock, and unlike lock/unlock, there was no existing intentional reason for `exams_officer` to be excluded — it reads as an oversight, not a deliberate security floor.

**Fixed:** `exams_officer` added to `TRANSITION_ROLES` for `in_progress`, `completed`, `cancelled`, `moderated`, `published`, and `archived` — full parity with admin/superadmin for the ordinary pipeline. `locked` and `approved` were deliberately handled differently, not by adding `exams_officer` to those two static lists (see §4a's fromStatus-aware split — blanket-adding to `approved` would have also silently opened the `locked → approved` unlock transition). Client (`ExamsPage.jsx`) updated to match: Moderate/Reopen/Approve/Publish/Archive now check `role === 'exams_officer'` alongside admin/superadmin, while Lock/Unlock buttons check the actual `exams__lock`/`exams__unlock` grant via the client's existing `can()` permission helper (which already reads sub-keyed grants the same way `hasExplicitSubGrant` does server-side) — so button visibility now mirrors real authorization instead of a second, independent role guess. 20 new tests, `exams-officer-transitions.test.js`. `teacher`'s scope (still only `in_progress`/`completed`) is unchanged and re-verified as a regression guard.
...
**Grades/Exams/Report Cards summary (updated 2026-09-03):** All three
identified BYPASS findings in this section are now **fixed**:
`grades.report_generate` (who can publish, commit `2d23079`),
`grades.mark_submissions` (who can review, commit `b9f5d85`), and
`exams.lock`/`exams.unlock` (§4a above, Option A). Each was a hardcoded
role check invisible to Settings, not a from-scratch DECORATIVE row as
originally assessed; all three now stay admin/superadmin-floored while
additionally being grantable through Settings via `hasExplicitSubGrant`.
`exams.lock`/`unlock` required the most involved trace (four overlapping
mechanisms) and a mandatory legacy-data audit before coding, since the
fix changed the meaning of an already-populated key — see §4a for the
real production impact found.

**Update (2026-09-05):** the `PUT /:id` alternate path noted above as
unresolved is now fixed for the PERMISSION dimension — see §4a's
"RESOLVED" addendum, which also documents that the 2026-09-03 fix was
itself incomplete (the dedicated `/lock` endpoint didn't actually work
for a granted non-floor caller either, despite the trace above stating
otherwise). The reason/audit-log request-shape asymmetry on `PUT /:id`
remains separately open, deliberately untouched. §4b documents a
distinct, larger finding from the same review: `exams_officer` was
excluded from `TRANSITION_ROLES` for every transition, not just lock/
unlock — now fixed for the ordinary pipeline (Start/Complete/Moderate/
Approve/Publish/Archive/Cancel), with lock/unlock kept on the stricter
floor-or-explicit-grant standard §4a already established.
...
- ✅ **DONE (2026-09-03)** `exams.lock`/`unlock` — see §4a for the
  complete authorization trace (four overlapping mechanisms) and the
  Option A fix actually implemented: dedicated `POST /:id/lock` and
  `POST /:id/unlock` reconnected to Settings via two independently
  grantable rows (registry split from one combined key). A mandatory
  legacy-data audit ran before coding, since the fix changed an
  already-populated key's meaning — real impact found and recorded in
  §4a. The `PUT /:id` alternate path (weaker requirements, same
  transitions) is explicitly **not** resolved by this fix — tracked
  as its own open finding, not silently dropped.
