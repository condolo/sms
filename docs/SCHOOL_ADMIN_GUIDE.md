...
5. **Admission Number** *(2026-09)* — leave blank to auto-generate the next number in your school's sequence, or type one in if this student already has one (a physical register, a previous system). Only fill this in when they already have a real number — typing one in doesn't reserve it for later, and a number already in use by another student is rejected right away rather than silently duplicated.
6. Save — student is immediately active

### Via Admissions

For a formal application workflow, **Admissions → New Application** tracks a child through nine pipeline stages: Enquiry → Application → Assessment → Interview → Offer → Acceptance → Enrolled (Withdrawn/Rejected close the pipeline at any point). **Full Names** are always required. By default, so are **Gender, Date of Birth**, and **at least one parent** (Mother or Father — see below) — but each of those four is a toggle your school controls (see below), not a fixed rule. Optional either way: House selection, Allergies, Emergency Contact, ID/Passport numbers for either parent, previous school.

**Mother and Father are entered separately**, each with their own name, email, phone, and ID/Passport number — not a single combined "parent" field. **By default, email is required for any parent you name** (phone is optional) — this is what lets that parent get their own portal login later (see Student portal accounts, below); a parent entered with a name but no email will be rejected at save time, unless your school has relaxed this. Pick which parent is the **Primary Contact** — that one drives the "Registration Date"/"Start Date" style school communications; it doesn't limit which parent can later get a portal account.

> **Choosing what's required.** Go to **Settings → School Profile → Admission Requirements** to turn Gender, Date of Birth, "at least one parent," and "named parent's email" on or off independently for your school. Every toggle defaults to on (the behaviour above), and whatever you choose applies the same way here, on the bulk-import CSV below, and at the enroll step — one setting, everywhere, not something you'd need to configure three times.
...
Available from **Students → Import**. Upload a CSV of up to 500 students per batch — the system validates every row first and, right there in the same panel once it finishes, reports how many were created, how many rows matched a student who already exists (by admission number — left unchanged, not updated, not an error), and the exact reason for any row that was genuinely rejected. Rows that don't supply their own admission number are auto-assigned the next free number in your school's sequence — checked against every existing student first, so it can't collide even if some of your other students were imported with their own real-world numbers out of sequence.

The template mirrors the Admissions form field-for-field, including the same **Admission Requirements** settings (Settings → School Profile) — by default, Date of Birth and Gender are required on every row, and the same Mother/Father-with-mandatory-email rule applies (or use the older combined `parentName`+phone/email columns instead, if that's what your existing records use — either is accepted, but at least one must be filled in unless your school has turned that requirement off). House, Allergies, and Emergency Contact columns are optional and land in the same places as on the Admissions form. Opening fee columns are optional and only needed when migrating student balances from another system — download the template from the Import screen for the full column list with example rows.

### Finding and resolving duplicate students *(2026-09)*

If two student records ever end up sharing the same admission number — most often from data brought in before this platform started checking for it — a **Duplicates** button appears in the toolbar on the Students page (admins only), showing how many groups need attention. Opening it shows every group side by side: name, class, gender, house, status, parent contact, how many invoices/payments are attached to each, and when each was added. One record in each group is marked **Suggested** — whichever has more invoices/payments attached (the one actually in use), or whichever was added first if neither has any — but that's only a suggestion; review each group before removing anything.

Every group opens pre-checked and pre-selected on its suggested record, so a school with many duplicates doesn't mean reviewing them one at a time — uncheck any group you're not ready to resolve yet, or click a different student card in a group to keep that one instead (its **Keeping** badge moves with your click). Use **Select all** / **Deselect all** to work through the whole list quickly. One **Remove Duplicates** bar at the bottom shows the running total ("3 groups, 5 records") and asks for a single confirmation before resolving every selected group in one go.

**What actually happens on confirm:** the removed record's attendance, exam results, behaviour history, invoices, payments, and everything else tied to it are moved onto the kept record first — nothing is left behind and nothing real is thrown away — and only then is the now-empty duplicate record itself permanently deleted. This cannot be undone, so take a moment on each group to check which record actually has the right class, house, and parent contact before confirming — the more complete one isn't always the one the system suggests. One thing worth knowing: if both records already had their own entry for the same thing (say, two separate invoices for the same term), the kept record ends up with both after merging — a quick look in Finance afterward catches this.

### Editing a student's details, including admission number *(2026-09)*

Open **Students → [student] → Edit** to correct any field, including the **Admission Number** itself — useful for fixing a typo, or reconciling a record with the number the student actually has on their physical file. Changing it is checked the same way a new one is at enrollment: a number already in use by another student is rejected with a clear error rather than silently creating the kind of duplicate the Duplicates tool above exists to clean up.
...
**Onboarding a whole school at once** *(2026-09)*: instead of selecting students one by one, click **Activate All Portals** (top toolbar) to create an account for every active student who doesn't already have one — useful right after importing a full student list. It works exactly like the selection-based flow above (same random one-time password per student, same forced change on first login, same automatic credentials CSV) — an account that's already active is never touched or reset, whichever way you grant access. If your organisation has more than one school sharing one login, this covers whichever school is currently selected — switch schools and run it again for the other one.
...
*Last reviewed: 2026-09-11 — §7's "Student portal accounts" gained an "Onboarding a whole school at once" note for the new Activate All Portals action, checked directly against `server/routes/students.js` and `client/src/pages/students/StudentList.jsx`. Earlier the same day: §7's "Finding and resolving duplicate students" corrected: resolving a group now merges the removed record's attendance/exam/behaviour/finance history onto the kept one instead of only cleaning up invoices and payments, checked directly against `server/utils/student-merge.js` and `server/routes/students.js`. Earlier the same day: §7 gained an "Editing a student's details, including admission number" subsection — the field on the edit form had always been there but silently discarded any change, checked directly against `server/routes/students.js`. Earlier the same day: §7's "Finding and resolving duplicate students" updated for bulk selection: every group now opens pre-selected, with Select all/Deselect all and one shared confirm-and-remove bar instead of a per-group confirmation, checked directly against `server/routes/students.js` and `client/src/pages/students/StudentList.jsx`. Earlier the same day: §7 gained that subsection for the new Students-page Duplicates tool. Earlier: 2026-09-10 — §7's "Individual enrollment" gained a step for the new optional Admission Number field, and the Bulk import (CSV) paragraph's "can never collide" claim was made actually true (auto-generated numbers are now checked against every existing student, not just the counter's own position) — checked directly against `server/utils/counters.js` and `server/routes/students.js`. Earlier the same day: §8's "Renaming a role" wording ("shown throughout the app") re-verified after HR's staff list and a user's own Profile page were wired up to the same rename — previously only Settings itself reflected a rename, checked directly against `client/src/pages/hr/HRPage.jsx` and `client/src/pages/profile/ProfilePage.jsx`. Earlier the same day: §8 gained a "Renaming a role" subsection: built-in roles (Teacher, Finance, etc.) can now be renamed, not just custom ones, checked directly against `client/src/pages/settings/SettingsPage.jsx` and `server/routes/settings.js`. Earlier the same day: Bulk import (CSV) section updated: a duplicate admission number is now reported as "already exists" rather than silently creating a second student record, checked directly against `server/routes/import-export.js`. Earlier: 2026-09-07 — §7 updated for the new per-school Admission Requirements settings (Settings → School Profile). Also checked directly against the live server code (plan tiers, Admissions/Mother-Father fields, portal accounts), not carried forward from a prior draft. §2 and §6 corrected 2026-09-05 — staff creation via HR → Staff had been omitted entirely in favor of Settings → Users alone, and the Settings button name itself was wrong ("Add Staff" — the real button is "Invite User").*
