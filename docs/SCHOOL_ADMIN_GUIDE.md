...
If two student records ever end up sharing the same admission number — most often from data brought in before this platform started checking for it — a **Duplicates** button appears in the toolbar on the Students page (admins only), showing how many groups need attention. Opening it shows every group side by side: name, class, gender, house, status, parent contact, how many invoices/payments are attached to each, and when each was added. One record in each group is marked **Recommended** — whichever has more invoices/payments attached (the one actually in use), or whichever was added first if neither has any — but that's only a suggestion; review both before deciding.

Clicking **Keep this one** asks for one explicit confirmation naming exactly what will be removed, then permanently deletes the other record(s) in that group along with any invoices and payments attached to them. This cannot be undone, so take a moment to check which record actually has the right class, house, and parent contact before confirming — the more complete one isn't always the one the system recommends.
