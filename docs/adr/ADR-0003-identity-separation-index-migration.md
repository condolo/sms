...
**Implementation:** **Phase 0 (Shadow) complete.** `identities` collection + indexes, `provision-identities.js` (collision policy, 12 unit tests), boot wiring, inline hooks at all 13 AST-verified `users`-creation sites. Full suite: 26 test suites, 306/306 passing. **Not yet consulted anywhere** — `auth.js` still authenticates against `users.password` exclusively. Phases 1-3 (Dual-write, Verify, Cutover) have not started; per this ADR's own adoption gate, each remaining phase is independently verified before the next begins — Phase 1 is the next checkpoint, not yet started.
