...
**Implementation:** **All 4 phases (Shadow → Dual-write → Verify → Cutover) code-complete.** Phase 3 (Cutover): the credential check in `/login`, `/change-password`, and `PUT /api/settings` now reads `identities.passwordHash`/`mfaEnabled` when a user has an `identityId` — but **only when `IDENTITY_CUTOVER_ENABLED=true` is explicitly set**, an opt-in kill switch (`server/utils/identity-cutover.js`) disabled by default. **Merging this code changed nothing in any running deployment** — the actual behavioral cutover is a separate, later, operator-controlled decision (see Adoption Gate). A dangling `identityId` or unusable `passwordHash` fails closed to a credential mismatch rather than silently falling back to `users.password` — a real bug in the first implementation, caught by this phase's own tests (see Consequences). Full suite: 32 test suites, 366/366 passing (17 new tests for Phase 3 alone; 63 total across Phases 1-3).
...
**Bugs the implementation's own tests caught, not manual review:** (1) the first version of the cutover read logic used one nullable variable to mean both "identity lookup wasn't attempted" and "identity lookup found nothing" — a dangling `identityId` silently fell back to `users.password` instead of failing closed, exactly the behavior this ADR's Decision 3 explicitly said not to build. Fixed by tracking `identityLookupAttempted` as a separate boolean at all three cutover sites. (2) A pre-existing bug in `auth-session.test.js` (not introduced by this ADR): one test permanently overrode the `_model` mock's implementation via `.mockImplementation()`, which `jest.clearAllMocks()` does not reset — every later test in the file silently inherited the override. Latent until Phase 3's new tests needed the original `identities`-aware mock; fixed by switching that test to `.mockImplementationOnce()`.
...
**Code-complete is not the same as live.** All four phases are implemented and merged, but the actual behavioral cutover — the moment `identities.passwordHash` genuinely becomes authoritative for any real login — is gated behind `IDENTITY_CUTOVER_ENABLED`, unset by default. Turning it on in any real deployment is a separate, later, operator decision that this ADR does not make and should only happen after: (1) `GET /api/qa/health`'s `identity` gate reports `status: 'complete'` against that deployment's real data, and (2) the dual-write from Phase 1 has been running long enough that the operator is confident no divergence exists. Rolling back at that point is unsetting the env var — instant, no code revert, no redeploy.
