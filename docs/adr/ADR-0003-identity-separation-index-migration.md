...
**Implementation:** **Phases 0-1 complete.** Phase 0 (Shadow): `identities` collection + indexes, `provision-identities.js` (collision policy), boot wiring, inline hooks at all 13 AST-verified `users`-creation sites. Phase 1 (Dual-write): two-tier revocation (`getIdentityTokenVersion`/`revokeIdentityTokens` in `token-version.js`, additive `itv` check in `authMiddleware`), `_buildTokenPayload` gains `identityId`/`itv` (async, 5 call sites), all 4 password-write paths (`/change-password`, `/force-change`, `PUT /api/settings`, admin `reset-password`) now dual-write the identical hash to `identities.passwordHash` and revoke both tiers — closing a pre-existing gap where none of the 4 ever revoked any session. Full suite: 30 test suites, 337/337 passing (this phase alone added 33 new tests across 4 files, the first coverage any of these paths, `authMiddleware`'s `tv` check, or `token-version.js` ever had). **Still not consulted for authentication** — `auth.js`'s credential *check* still reads `users.password` exclusively; only the *write* path is dual-writing now. Phases 2-3 (Verify, Cutover) have not started; per this ADR's own adoption gate, each remaining phase is independently verified before the next begins.
