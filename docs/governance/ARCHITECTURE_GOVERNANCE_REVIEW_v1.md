...
| BUG-003 | Exam mark-entry (`POST /api/exams/:id/results`) bulk-upserts with no version check. Two teachers saving marks for the same student within the same window silently last-write-wins — no conflict surfaced to either party, no audit trail of the overwritten value. | High | **Fixed, both halves — but the endpoint itself turned out to be dead code.** *Corrected 2026-07-19.* Server: `ResultSchema` gained an optional `_v`, a stale version excluded from the write and reported in `conflicts`. Client: `client/src/pages/grades/components/ExamResultsTab.jsx` now reads/sends `_v` and surfaces `conflicts` in a banner — but this component is not rendered by any route (`client/src/pages/exams/ExamsPage.jsx`'s own header comment: *"Markbook: schedule-driven unified mark entry (replaces Results + CA Marks)"*). The fix is correct and complete for the code as written; it just has no live path to a teacher. See **BUG-004** for the version of this defect that actually reaches production today. Regression test: `server/__tests__/routes/exams-mark-conflict.test.js` |
| BUG-004 | Discovered while verifying BUG-003's client-side fix reached a real user — it didn't; `assessment_marks`' bulk-save (`POST /api/assessment/marks/bulk`, the endpoint the *live* "Markbook" tab actually calls) had the identical defect BUG-003 described, undocumented until now: a plain `bulkWrite` of `$set` operations, no version field, no conflict detection. Two teachers entering CA/HW/MT/ET marks for the same student concurrently would silently last-write-win. | High | **Fixed** — `MarkSchema` gained the same optional `_v` field as `ResultSchema`; existing marks are pre-fetched by composite key (`studentId+subjectId+termNumber+assessmentType+instance+academicYearId`, since uniqueness here isn't a bare `studentId`), a stale version excluded from the write and reported in `conflicts`. Client: `ExamsPage.jsx`'s `MarkbookTab` (the actual, reachable Markbook) now tracks `_v` per cell, sends it on save, and surfaces conflicts in a banner + red-flagged cell, without clearing the teacher's unsaved entry. Verified end-to-end in-browser with injected mock responses (no live MongoDB in this sandbox). Regression test: `server/__tests__/routes/assessment-mark-conflict.test.js` |
...
**In parallel, gated by nothing above:** BUG-002, BUG-003, and BUG-004 — all fixed, all with regression tests, no open follow-up remaining. BUG-003's own fix reached only dead code (`ExamResultsTab.jsx`, unreachable from any route); BUG-004 is the identical defect in the endpoint the live Markbook tab actually calls, found while verifying BUG-003's client half actually protected a real user — it didn't, until BUG-004's fix.
