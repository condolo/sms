...
> **Connector Registry researched and deliberately deferred, 2026-07-18 — do not re-investigate without new information.** Checked directly whether building it now would consolidate real duplication: it wouldn't. `elearning.js`'s Google Classroom OAuth token store (`elearning_tokens`) is the only real "connector" today, with exactly one refresh implementation (`_refreshIfNeeded`), not duplicated anywhere. M-Pesa's credentials are a static secret embedded on `schools` (no token, no expiry, no refresh cycle) — structurally incompatible with an OAuth-token registry shape. Zoom's credentials are global env vars, not per-school at all. Google/Microsoft *login* OAuth (`auth.js`) discards its token immediately after reading the user's email — never persisted, not a connector in this sense. A registry today would hold exactly one real tenant and simplify none of `elearning.js`/`mpesa.js`/`auth.js` by a single line. Revisit only when a real second OAuth-token-with-refresh integration is actually being built (a second e-learning provider, QuickBooks, etc.) — building it speculatively now was explicitly rejected as the kind of premature abstraction ADR-0006 itself declined to do.
