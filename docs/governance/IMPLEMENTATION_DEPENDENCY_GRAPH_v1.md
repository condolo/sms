...
| C8 Identity separation + index migration | 🟡 **ADR-0003 approved 2026-07-18. Phase 0 (Shadow) shipped**: `identities` collection + indexes (`{orgId,email}` partial-unique, `collisionKey` partial-unique for `collision_pending`), `provision-identities.js` (`provisionIdentityForUser`/`provisionIdentities`, never-auto-merge collision policy, 12 unit tests), chained into boot after `provisionMemberships()`, inline hooks at all 13 AST-verified `users`-creation sites (11 production, 2 seed scripts deliberately left to the batch backfill's self-heal). **Still NOT consulted anywhere** — `auth.js` authenticates against `users.password` exclusively; `rbac.js`/`scopeMiddleware.js` untouched, confirmed unchanged. Phase 1 (Dual-write — touches `/change-password`) is the next checkpoint, per the ADR's own "each phase independently verified before the next begins" adoption gate — not started. |
