...
| C11 Integration framework | ⬜ **Explicitly deferred, by choice, 2026-07-18** — confirmed still blocked on queue infrastructure that doesn't exist (`node-cron` only; no Redis/BullMQ/Agenda; no retry-queue semantics anywhere in `server/`), matching this row's own stated dependency and the governance corpus's own recommendation to keep it deferred until a concrete integration justifies the investment. Not started. |
