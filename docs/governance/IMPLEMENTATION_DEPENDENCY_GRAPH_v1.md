...
| C9 onward (School Switching, D-004) | ⬜ Gated — requires C8 *authoritative*, meaning `IDENTITY_CUTOVER_ENABLED=true` in a real deployment with the `identity` gate confirmed green, not merely the code being merged with the switch off |
