...
| C8 Identity separation + index migration | 🟡 **Phases 0-1 shipped.** Phase 0 (Shadow): `identities` collection + indexes, `provision-identities.js` (never-auto-merge collision policy), boot wiring, inline hooks at all 13 AST-verified `users`-creation sites. Phase 1 (Dual-write): all 4 password-write paths (`/change-password`, `/force-change`, `PUT /api/settings`, admin `reset-password`) dual-write the identical hash to `identities.passwordHash` and revoke both `users.tokenVersion` (always — closes a pre-existing gap) and `identities.tokenVersion` (when `identityId` is set); `authMiddleware` gained an additive `itv` check; `_buildTokenPayload` gained `identityId`/`itv`. **Still NOT consulted for authentication** — `auth.js`'s credential *check* still reads `users.password` exclusively, only the *write* path dual-writes now; `rbac.js`/`scopeMiddleware.js` untouched. 33 new tests, first coverage these paths or `token-version.js` ever had. Phase 2 (Verify — extends `qa-health.js`'s gate pattern) is next, per the ADR's own "each phase independently verified before the next begins" adoption gate — not started. |
| C9 onward (School Switching, D-004) | ⬜ Gated — requires C8 *authoritative* (cutover complete), not merely designed |
