...
| C8 Identity separation + index migration | 🟡 **Phases 0-2 shipped.** Phase 0 (Shadow) + Phase 1 (Dual-write, two-tier revocation) as before. Phase 2 (Verify): `qa-health.js` gained an `identity` gate (`_identityMigrationStatus()` — deliberately excludes `collision_pending` users from "pending," since that's a permanent safe fallback, not an unfinished step) plus `_checkDanglingIdentityFK`/`_checkPasswordHashMismatch` integrity checks (null-normalized so OAuth users never false-positive), all attached on the router for direct unit testing. **Still NOT consulted for authentication** — only the write path dual-writes and the verify gate reports on it; nothing reads `identities` to authenticate anyone. 46 new tests total (Phases 1-2), first-ever coverage for `qa-health.js`, the 4 password routes, `authMiddleware`'s `tv` check, and `token-version.js`. Phase 3 (Cutover) is next — may not begin while the `identity` gate is red, per the ADR's own adoption gate — not started. |
