...
workflow_configs
  { id, schoolId, workflowKey, steps: [
      { order: 1, assigneeType: 'role' | 'user', assigneeValue, fallback: { assigneeType, assigneeValue } | null },
      { order: 2, assigneeType: 'role' | 'user', assigneeValue, fallback: null },
...
- `steps` — an **ordered array the school itself authors**, not a fixed set of named slots (`hod`/`principal`/etc. no longer appear as required keys anywhere in code). `assigneeValue` is a **stable reference, never a copied display string**:
  - `assigneeType: 'role'` → `assigneeValue` is a `roleKey` — the exact same identifier `role_permissions` and `custom_roles.key` already use as their canonical identity. This is not a new reference scheme; it's the one the RBAC system already treats as authoritative.
  - `assigneeType: 'user'` → `assigneeValue` is a real `users.id` (the same identifier `AuditService`, `memberships`, and every other cross-reference in the codebase already uses) — covers ad hoc roles like "Director" that a school may not want to formalize as an RBAC role at all.
  - **No `label`/display-name field is stored on a step.** A first draft of this spec included one (`label: 'First Approval'`) — a real mistake, caught on review: `custom_roles` documents have their own independently editable `label` (`PATCH /api/settings/custom-roles/:key` can rename it at any time) and can be deleted outright (`DELETE /api/settings/custom-roles/:key`). Freezing a copy of that name into `workflow_configs` at save time means the config silently shows a stale name the moment someone renames the role in Settings, and dangles with no defined meaning if the role is deleted. Any UI showing "who approves step N" resolves the current display name **live**, at render/query time, from `custom_roles.label` (for custom roles) or the existing built-in-role display convention already used in several client pages today (for built-in roles) — never a value copied into the config itself. This is the one place in this spec where "reference, don't copy" actually matters for correctness, not just tidiness — a copied name is silent data corruption waiting for the first rename.
- `assigneeType: 'role'` resolves to whoever holds that `roleKey` at the relevant scope; `assigneeType: 'user'` is a specific named person.
- `fallback` — per-step, optional. Used when a step's primary resolution finds nobody assigned — **and this now has to cover two real cases, not one**: a vacant role (nobody currently holds that `roleKey`), and a **dangling reference** (the referenced custom role was deleted, or the referenced user was deactivated/deleted after the workflow was configured — see §4's deactivation semantics, which this step resolution must check against live, the same way it checks role-holder vacancy). Both resolve through the same configured `fallback`, if one exists; if none exists, the step is flagged for the workflow-owner's attention rather than silently stalling the request. There's no platform-wide default fallback chain — the school configures what/who a given step falls back to, if anything.
...
A school's own workflow-owner configures this once, in Settings, populated from a picker over that school's actual role catalogue, not a hardcoded dropdown of platform-assumed role names — resolving live from `custom_roles`/the built-in role list at the moment the picker is opened, for the same reason the stored config never freezes a name. New business logic resolves *"who acts at step N"* by reading this collection's `steps` array in order — never by checking `role === 'hod'` or any other specific name in code. This is additive (`role_permissions` and `rbac()` are untouched), reuses the exact tenant-scoping and caching pattern already proven there, and is the concrete mechanism that makes "future modules — procurement, disciplinary cases, inventories, finance — follow the same flexible governance model" actually true rather than aspirational.

**Who may edit a `workflow_configs` document is itself a permission, not a hardcoded role check** — consistent with this section's own principle. §1 confirms leave-workflow editing is HR-only; that's expressed as an `rbac('hr','manage_workflow')`-style permission gate (a new, specific permission action on the existing `hr` module, resolved through the same `role_permissions`/per-user-override mechanism every other permission already goes through), not a literal `role === 'hr'` check in the route handler — which matters in practice, since it means a school could later grant this specific capability to a different person via the existing per-user override, without this spec having quietly reintroduced the hardcoded-role-name problem it's trying to fix.
...
**The chain itself is school-configured, not fixed — and editing it is HR-only.** Configuring the leave approval chain for a school is restricted to whoever holds the `hr.manage_workflow` permission (§0) — confirmed explicitly: not admin/superadmin by default, not every HR-permission holder for other HR actions, specifically the workflow-management permission, which happens to default to the `hr` role but is a distinct grantable permission, not the same as general `hr` module access. The platform enforces exactly one structural rule on what HR configures — **a minimum of two approval steps before HR's own final confirmation** — and nothing about *which* roles fill them, whether a Principal is involved at all, or what they're called. A school with no Principal role, or one that calls its department heads something else entirely, configures a chain that reflects that, and the workflow logic never assumes otherwise.
...
```
step[1] → step[2] → ... → step[N]   (N ≥ 2, school-defined)
   ↓          ↓               ↓
...
**Business rules:**
- Every step transition (approve or reject) writes an `AuditService.log()` entry — `action: 'leave.step_approved' | 'leave.step_rejected' | 'leave.hr_confirmed' | 'leave.hr_rejected'`, `actor`, `target: {type:'leave_request', id}`, `details: {comment, stepOrder, resolvedRoleLabel}`. `resolvedRoleLabel` is deliberately a **snapshot** — the display name as it actually resolved *at the moment of this specific decision* — unlike `workflow_configs` itself (§0), which must never store a copied name. This mirrors a pattern the codebase already uses correctly elsewhere (`growth_projects`/`growth_recommendations` denormalize a supervisor/author name alongside their id) for exactly this reason: an audit entry describes a historical event and should read correctly forever, even if the role is later renamed or the approving user's account is later deactivated — whereas the *live config* must never go stale relative to Settings/Users, which is precisely the opposite requirement. This is new — no leave action is audited today. Rejection comment is mandatory at the API layer (400 if omitted on a reject action), matching the mandatory-reason pattern already used by `mark-submissions.js`'s unlock endpoint.
...
5. **§1 — Is there a maximum step count.** *(Resolved in part: chain-editing is gated by a dedicated `hr.manage_workflow` permission, not a hardcoded role — see §0/§1. Still open:)* the floor is ≥2 steps before HR; is an unbounded number of steps above that floor actually desired, or should there be a sane maximum (e.g. 5) to prevent an unusably long chain being configured by mistake?
