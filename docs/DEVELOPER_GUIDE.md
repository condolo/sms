...
### Known limitation — required is enforced client-side only, flagged not fixed
`LessonPlanSlideOver` blocks submission locally if a required field (builtin or custom) is empty, but `POST /plans`'s `LessonPlanSchema` does not itself re-check a field's `required` flag from the live template before accepting the request. A direct API call, or a client that hasn't refreshed its template cache, can still create a plan missing a field the school just marked required. Not a regression — no version of this schema has ever had server-enforced required/optional distinctions for these fields — but worth closing in a follow-up now that "required" is a real, saved concept.
