...
**Billing-sequence fix (2026-09, resolves the limitation found in the self-audit above):** `POST /admissions/:id/enroll` now calls `server/utils/guardian-linking.js`'s `linkExistingGuardians()` right after creating the student, BEFORE `generateEnrollmentInvoices()` — so `resolveAutoDiscounts()`'s sibling calculation has the guardian's `studentIds` array already complete when it runs. `linkExistingGuardians()` deliberately never creates a new guardian account (that stays `POST /students/:id/parent-account`, a bigger credential-issuing action) — it only links onto one that already exists, via `siblingStudentId` (the application's own field, previously unread anywhere) or an email match. `ApplicationSchema.isDirectorFamily`/`isReferralFamily` close the equivalent gap for those two discount types — set at the application stage, carried onto the Student record at enroll time, instead of only settable afterwards. See CHANGELOG.md's v5.66.0 entry.
