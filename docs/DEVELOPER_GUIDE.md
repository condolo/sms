...
| `GET/POST/PUT/DELETE /api/finance/discount-policies` | premium | `discount_policies` — types `sibling` (tiered by nthChild), `director`/`referral` (flat `flatPct`, gated by `students.isDirectorFamily`/`isReferralFamily`); one active policy per TYPE per school; `_resolveAutoDiscounts()` computes all three inside `/fee-structures/:id/generate` and keeps only the highest per student — never stacked |
