...
`subject_paired` (RCE3, "Light International style," the recommended default) shares a cover-page builder (`_renderCoverHtml`/`_drawCoverPdf`, built from `_computeReportSections`'s named `cover` fields, not the generic `rows` list `legacy_tabular` uses) with `marks_then_comments` (RCE4). Its academic page pairs each subject's marks row with that subject's own teacher comment directly beneath it — the layout's whole reason for existing — respecting every RCE1 toggle (`resultsTable.showDeviation`, `summary.showAverage`/`showGPA`/`showRanking`, `comments.showClassTeacherRemark`/`showPrincipalRemark`, `subjectTeacherCommentsEnabled`) that `legacy_tabular` ignores by design.
